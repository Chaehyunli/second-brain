---
title: "[8/24] 컨테이너 이해 및 애플리케이션 컨테이너화_Day2_핵심 정리"
notion_page_id: "3c51d84b-f68e-80cd-b1a0-ca86dc384108"
source_url: "https://app.notion.com/p/3c51d84bf68e80cdb1a0ca86dc384108"
synced_at: "2026-08-24T15:10:45+00:00"
content_sha256: "20826bc7537842836a3bbdfd4641e779db6334df4828d2330b21c7a76243c9c4"
---

# [8/24] 컨테이너 이해 및 애플리케이션 컨테이너화_Day2_핵심 정리

[[notion/SKALA/index|SKALA 학습 노트]]

> 원문: [Notion 페이지](https://app.notion.com/p/3c51d84bf68e80cdb1a0ca86dc384108)
>
...
#### 부동산 비유 → 기술 레이어 대응
| 비유 | Bare Metal | VM | Container |
| --- | --- | --- | --- |
| 건물 구조 | 1000평 단독주택 (통째) | 다세대 주택 (세대별 분리) | 단독주택 (방 공유) |
| 화장실·부엌 | 전체 공유 | 세대마다 개별 | 방끼리 공유 |
| 도둑 침입 시 | 전체 위험 | 그 세대만 털림 | 전체 위험 |
| 격리 엔진 | 없음 (물리 머신 그대로) | Hypervisor | Container Engine |
| Guest OS | 없음 | **VM마다 별도 Guest OS** | 없음 (호스트 OS 공유) |
#### 스택 구조
#### 핵심 특성 비교
| 기준 | Bare Metal | VM | Container |
| --- | --- | --- | --- |
| 격리 강도 | 없음 | 강 (OS 수준) | 중 (프로세스 수준) |
| 자원 효율 | 낮음 (통째 점유) | 낮음 (Guest OS 오버헤드) | 높음 |
| 기동 속도 | 분\~수십 분 | 분 단위 | 초 단위 |
| 이미지 크기 | 해당 없음 | 수 GB (OS 포함) | 수십\~수백 MB |
| 보안 경계 | 물리 머신 전체 | VM 단위 | 호스트 OS 단위 |
| 스펙 예시 | 128 Core, 512 GB | VM 단위 할당 | 프로세스 단위 할당 |
#### 언제 무엇을 쓰는가
...
실무에서는 **VM 안에 Container를 띄우는 혼합 구조**가 흔함 → VM이 격리 경계를 담당하고, 그 안에서 Container가 경량 배포를 담당하는 역할 분담
---
...
- 트래픽 유실 및 배포 속도 저하
핵심 원인: **PID 1 프로세스가 SIGTERM을 수신하지 못하는 구조**로 CMD를 작성한 경우 발생함
---
#### CMD 세 가지 형태 비교
| 형태 | Dockerfile 예시 | PID 1 | SIGTERM 수신 | 권고 |
| --- | --- | --- | --- | --- |
| Exec Form (직접 실행) | `CMD ["python3", "webserver.py"]` | python3 직접 | O | 강력 권고 |
| Shell Form (포크 실행) | `CMD ["/bin/sh", "-c", "python3 webserver.py"]` | /bin/sh | X (앱은 자식 프로세스) | 비권고 |
| Shell with Exec (치환 실행) | `CMD ["/bin/sh", "-c", "exec python3 webserver.py"]` | python3 (sh→앱으로 치환) | O | 조건부 권고 |
---
...
# exit
```
- PID 1 = `/bin/sh` → SIGTERM이 sh에만 전달되고, 자식인 python3에는 전달되지 않음<br>→ 이전 `Exec Form(직접 실행)` 방식에서는 “`python3 webserver.py`”가 PID 1이어서 바로 SIGTERM을 받아서 죽었었음.(프로그램을 안전하게 정리하는 Graceful Shutdown)<br>→ `Shell Form(Fork 실행)`에서는 /bin/sh 가 SIGTERM을 받아 자식에게는 전달 x(자식은 10초간 기다리다가 강제 종료 명령어로 종료가 되기 때문에 데이터 유실의 문제가 발생 가능)  
...
# exit
```
- `exec` 명령어로 sh 프로세스 자체를 python3으로 교체 → PID 1 = python3<br>→ `exec`는 "새로운 자식 프로세스를 띄우지 말고, 현재 쉘 프로세스(PID 1)를 그 프로그램으로 완전히 덮어씌움(Replace)”<br>→ 컨테이너가 켜질 때 처음에는 `/bin/sh`가 **PID 1**로 시작<br>→ 하지만 `exec` 키워드 때문에 `/bin/sh`가 실행되자마자 자기 자신을 **`python3`**** 프로세스로 교체**<br>→ 결과적으로 `/bin/sh`는 사라지고, **`python3`****가 PID 1번을 물려받게 됨.**
...
터미널을 연다 = **Stdin/Stdout과 연결된 Shell Process를 실행**하는 것임
| 명령 | 내부 동작 |
| --- | --- |
| `python fastapiserver.py` | Shell → Child fork + exec(치환) + wait(대기) |
| `python fastapiserver.py &` | Shell → Child fork + exec (wait 없이 백그라운드) |
| `cat "hello"` | Shell → Child fork + exec(cat) |
| `docker exec -it container /bin/bash` | 컨테이너 내부에 새 Shell Process 생성 |
...
- `CMD`에서 `exec`를 사용 → Shell with Exec 방식으로 java 프로세스가 **PID 1**을 차지 → SIGTERM 수신 가능 (앞 챕터와 연결)
#### 빌드 및 실행
```bash
# 1. Maven 빌드 (jar 생성)
...
`mvn build`와 `container build`를 하나의 Dockerfile로 처리하는 방식. CI 환경에서 로컬에 Maven 없이도 빌드 가능함
...
```
- 빌드 스테이지: ubuntu에 JDK + Maven 설치 → `mvn package`로 jar 생성
...
Spring Boot의 Thymeleaf 코드를 **외부 SPA 구조의 컨테이너로 전환**해서 별도 배포
...
- `resolver 127.0.0.11` — Docker 내부 DNS 서버. `set $backend`로 변수화해야 컨테이너가 뜰 때 backend가 없어도 nginx가 죽지 않음
- `/api` 경로를 `spring-backend:8080`으로 프록시 → 브라우저는 같은 호스트로만 요청하면 됨 (CORS 우회 효과)
#### Dockerfile
```docker
FROM nginx:alpine
# 기본 nginx 설정 파일 제거
...
docker run -d \
  ...
  frontend:1.0
```
- `-network skala` — spring-backend와 동일 네트워크 → nginx가 `spring-backend:8080`으로 프록시 가능
...
`default.conf`는 앞의 정적 Frontend와 동일하게 사용함
Vue.js는 소스코드(`.vue`, `.js`)를 그대로 서빙하지 않고, **`npm run build`****로 컴파일한 static resource**를 컨테이너에 담음
...
CMD ["nginx", "-g", "daemon off;"]
```
- 빌드 스테이지: `node:20-alpine`에서 `npm run build` → `/app/dist`에 정적 파일 생성
- 배포 스테이지: `nginx:alpine`에 dist 결과물만 복사 → Node.js, node_modules가 최종 이미지에 포함되지 않음
#### 빌드 및 실행
```bash
# 이미지 빌드
...
- 브라우저의 JavaScript가 `/api`를 통해 `01.spring-backend-v1.0`을 호출함
---
### 컨테이너 이미지 구조
#### 컨테이너 이미지란
...
압축 해제 후 `manifest.json`을 보면 이미지의 전체 구성이 보임
```bash
...
- 같은 해시(`4f4fb700...`)가 두 번 등장 → **동일 레이어를 재사용**하고 있음 (빈 레이어인 경우 발생)
#### Config 메타데이터 확인
```bash
> cat blobs/sha256/5b666a89cbc7dd0abf11902346cc178118d055d8b8000fb29ecaaea6592d0491 | jq
```
`manifest.json`의 `Config` 경로를 따라가면 Dockerfile에 명시한 ENV, LABEL, EXPOSE, CMD 등이 JSON으로 저장되어 있음
---
### 각 Layer와 Dockerfile 명령어 대응
각 `blobs/sha256` 레이어를 `tar tvf`로 열면 어떤 Dockerfile 명령어가 만든 레이어인지 확인 가능함
| Layer (sha256 앞 8자) | 대응 Dockerfile 명령어 |
| --- | --- |
| `119d19e0...` | `FROM ubuntu:${UBUNTU_VERSION}` |
| `0655737b...` | `RUN apt-get update && apt-get install -y curl lsb-release nginx` |
| `4f4fb700...` (첫 번째) | `RUN echo "현재 빌드에 사용된 ubuntu version: ${UBUNTU_VERSION}"` |
| `4f4fb700...` (두 번째) | `WORKDIR /var/www/html` |
| `a1b97ad0...` | `COPY index.html .` |
```bash
...
| Layer 순서 | 대응 Dockerfile 명령어 |
| --- | --- |
| Layer 1 (`69c262...`) | `FROM ubuntu:jammy` |
| Layer 2 (`4a585e...`) | `RUN groupadd -r mysql && useradd ...` |
| Layer 3 (`986b70...`) | `RUN set -eux;l \ apt-get ...` |
| Layer 4 (`9bf066...`) | `RUN mkdir /docker-entrypoint-initdb.d` |
| Layer 8 (`6eb575...`) | `RUN set -e;\ echo "dev ...` |
#### 마지막 레이어 내용 확인
```bash
> cd ./blobs/sha256
...
```
마지막 레이어에 `docker-entrypoint.sh`가 들어 있음 → 컨테이너 시작 시 초기화 스크립트를 담당하는 파일임을 레이어 단위로 직접 확인할 수 있음
---
### Docker Architecture — 컨테이너 생성 흐름

- Client가 명령을 내리면 **Docker daemon**이 Registry에서 이미지를 pull해 로컬 Images에 저장하고, 그 이미지로 Container를 생성함
...
- **libcontainer (내장 라이브러리):** `runc` 내부에 내장된 핵심 패키지로, **리눅스 커널에 직접 명령을 내려 격리 공간을 생성**합니다.
...
- OCI 스펙의 `config.json`을 읽어 컨테이너 생명주기 관리
...
| 기술 | 역할 |
| --- | --- |
| Namespace | 프로세스마다 서로 다른 시스템 뷰 제공. PID, NET, MNT, IPC, USER, UTS/HOSTNAME |
| cgroups | CPU, Memory, Disk I/O throttle, 프로세스 개수 등 자원 사용량 제한·격리·모니터링 |
| rootfs + OverlayFS | 이미지 레이어를 모든 컨테이너와 재사용. 컨테이너마다 독립 파일시스템으로 격리 |
| Netfilter | 네트워크 패킷 가로채기·검사·변경. Port-forwarding, NAT, 컨테이너 간 통신 격리 |
| Linux Capabilities | root 권한을 세분화. CAP_NET_ADMIN, CAP_SYS_ADMIN, CAP_KILL 등 |
| SELinux/AppArmor | 프로세스가 어떤 대상(파일·포트·프로세스)에 접근할 수 있는지 강제 통제 |
---
### 권한 체계
...
| namespace | 기능 |
| --- | --- |
| PID | 컨테이너마다 별도의 프로세스 테이블을 관리 (프로세스 트리 분리: 컨테이너 안에서 PID 1부터 시작). 특정 컨테이너의 프로세스에서 다른 컨테이너의 프로세스로 접근할 수 없음 |
| Network | Virtual Ethernet (veth0), IP Address, 포트 번호, 라우팅 테이블, 필터링 테이블 등 네트워크 리소스를 namespace별로 할당 가능 |
| UID/GID | 독립적인 사용자 할당. User ID(UID)와 Group ID(GID) 공간을 격리. namespace 외부에 있는 user나 group이 내부의 공간을 보거나 건드릴 수 없음 |
| Mount | 마운트를 조작하여 namespace 내에 파일 시스템 트리를 구성하여, 컨테이너마다 특정 디렉토리를 루트 파일 시스템으로 보이게 함 (rootfs) |
| UTS | namespace별로 hostname과 domain-name을 분리 (file 분리: /etc/hosts, /etc/resolve.conf, /etc/hostname) |
| IPC | 공유 메모리, 큐, 세마포어 등 프로세스 간의 통신(IPC, Inter-Process Communication) 자원을 분리 할 수 있음 |
---
### 프로세스에 할당된 리소스 — Control Group: cgroup
동일 namespace에 할당된 프로세스 집합에 대해 CPU, Memory, I/O 등의 사용량을 **제한, 격리, 모니터링**하는 기능
...
| 항목 | 기능 | 자원 | 예 |
| --- | --- | --- | --- |
| cpu | CPU 사용량 제한 | CPU | quota, shares |
| cpuacct | CPU 사용량 통계 정보 제공 | Memory | limit, oom |
| cpuset | CPU와 메모리 배치 제어 | Block I/O | read/write throttle |
| memory | 메모리와 swap 사용량 제한 | PIDs | 프로세스 수 제한 |
...
1. Linux 커널의 **OOM Killer**가 동작 → 프로세스 강제 종료
2. shim이 PID 1 종료 감지 → container 종료 상태를 containerd에 알림
3. containerd가 container 상태 갱신 (Docker는 `Exited/137`, k8s는 `OOMKilled`)
...
| 자원 종류 | 컨트롤러 | 제어 방식 예시 |
| --- | --- | --- |
| CPU | cpu / cpuacct | cfs quota, period 로 제한 |
| Memory | memory | limit 초과 시 OOM kill |
| I/O | blkio | 읽기/쓰기 속도 제한 |
| PID 수 | pids | 프로세스 개수 제한 |
| Network | (없음) | cgroup 자체로는 직접 제한 불가 (tc 등 필요) |
---
...
| 구성요소 | 역할 |
| --- | --- |
| LowerDir (Read-Only) | 읽기 전용 레이어. 컨테이너 이미지 원본 레이어들 (수정/삭제 불가) |
| UpperDir (Read-Write) | 쓰기 가능 레이어. 컨테이너 실행 후 사용자 생성·수정·삭제 파일이 물리적으로 저장됨. 원래 lowerdir에 있던 파일을 수정할 경우 upperdir로 복사된 후 변경 |
| MergedDir (=rootfs) | LowerDir과 UpperDir을 OverlayFS로 병합하여 구성한 실제 호스트 상의 디렉토리. 해당 컨테이너의 독립된 루트 파일 시스템(/)을 의미 |
| workdir (작업 디렉토리) | 디렉토리 생성, 삭제, rename 작업 등에 필요한 임시 작업 공간 |
...
```
CoW 동작 확인: merged에서 `a.txt`를 수정하면 **lower의 원본은 그대로 유지**되고, **upper에 수정본이 새로 생성**됨
---
...
- dockerd로부터 받은 컨테이너 실행 설정을 `config.json`으로 구성
...
1. namespace 생성, cgroups 설정
2. pivot_root 로 / 교체
3. `execve()` init process 실행 및 init process PID shim에 전달
...
- `manifest.json` 표준
#### 2. OCI Runtime Spec
컨테이너 실행을 위한 **OCI Bundle**을 정의하는 표준 규격
```plain text
OCI Bundle
...
| 유형 | 생성 방식 | 목적 |
| --- | --- | --- |
| bridge | Default (docker0) | 리눅스 커널에 생성된 가상 브리지 (L2 switch). docker0 bridge network 기반 통신 |
| host | `docker run --network host` | 호스트 네트워크 직접 사용. Bridge 생성하지 않음, Veth0 없음, NAT 없음 |
| custom bridge | `docker network create my-bridge` + `docker run --network my-bridge` | 새로운 Bridge Network를 별도 생성 |
#### 네트워크 구조 다이어그램
...
---
### \[참고\] 패킷 변환 및 전달 흐름
HostIP: `192.168.1.22:888` → 컨테이너 IP: `172.17.0.3:8080`으로 전달되는 흐름
1. **Host 물리 NIC 진입 → Netfilter 최초 낚아채기 (PREROUTING)**
	- 외부 Client → eth0 → Netfilter PREROUTING → ipTable NAT 규칙 (8888 → 172.17.0.3:8080)
2. **Kernel Routing Table 검색 → docker0 결정**
...
3. **docker0 브리지 연결된 가상 NIC (Veth)로 중계**
...
| host | ✗ | ✗ | 최고 | 성능 최우선 |
Custom Bridge는 DNS를 지원하는데, 이것은 **Docker Network 내부에 DNS 서버가 자동으로 동작**하기 때문임. 컨테이너 이름(`myservice`)으로 다른 컨테이너에 접근 가능 → `curl myservice` 형태로 서비스 디스커버리 가능
...
`ipconfig / ifconfig`로 나의 IP를 확인하고 다른 사람 네트워크로 접속해보기
---
### Docker 컨테이너간 통신
Docker Network는 **단일 노드에서의 통신만을 정의**하고 있음
#### Docker 컨테이너간 통신 (링크 기능)
동일 호스트 내 bridge docker0에 접속한 컨테이너간 통신. `/etc/hosts`에 `db` 항목을 추가해 이름으로 접근하는 Link 방식 사용
...
Calico는 Kubernetes에서 사용하는 **CNI(Container Network Interface) 구현체**로, 멀티 노드 환경에서 Pod 간 네트워크 통신을 가능하게 하는 네트워크 플러그인
...
- nerdctl, docker, ctr은 **containerd**를 거쳐 runc로 컨테이너 실행
...
2. pivot_root를 통해 컨테이너의 root filesystem을 변경한 뒤
3. 해당 namespace 환경에서 프로세스를 실행한다
...
- `runc list`로 호스트(runc-test 컨테이너) 입장에서 runc가 관리하는 컨테이너 목록 확인 가능
...
314  root   0:00  sh
```
namespace에서 `"type": "pid"` 항목을 제거하면 → PID namespace가 격리되지 않음 → 컨테이너 내부에서 `ps -ef`를 실행하면 **호스트의 모든 프로세스가 보임** → docker의 `--pid=host` 옵션과 동일한 효과
---
...
> runc list
```
실습 의미: config.json의 `args`에 `/init.sh`를 넣으면 그것이 PID 1이 됨. 이것이 Dockerfile의 CMD가 결국 하는 일 — containerd가 config.json을 작성하고 runC가 이 파일을 읽어 init process를 실행하는 것임
---
### 컨테이너가 실행 단위인가?
컨테이너란?
containerd나 dockerd 등의 컨테이너 런타임이 정의한 **논리적 그룹**이며, Linux 커널의 namespace와 cgroup 기능으로 프로세스 그룹을 격리·제한하여 구현된 것
...
---
#### 컨테이너의 실행 주체는 프로세스
컨테이너는 **Host 환경의 파일 디렉토리와 프로세스로 동작**한다
<embed src=""></embed>
- 컨테이너는 별도의 OS가 아님. Host의 Linux Kernel 위에서 프로세스로 동작함
- 컨테이너가 보는 루트 파일시스템(`/`)은 OverlayFS + pivot_root로 교체된 것뿐
- HOST 프로세스와 Container 내 프로세스는 **동일한 커널**을 공유함
- 실습 3(PID namespace 제거)에서 확인했듯, namespace만 제거하면 컨테이너 경계가 사라지고 호스트 프로세스가 그대로 보임 → 컨테이너의 격리는 커널 기능으로 만든 논리적 경계임
---
### Docker-Compose란?
**단일 노드** 내에서 여러 개의 컨테이너를 조율하는 **컨테이너 오케스트레이션 플랫폼**임. 정확히는 여러 컨테이너로 구성된 애플리케이션을 **하나의 ****`docker-compose.yaml`**** 파일**로 정의하고, **한 번의 명령**으로 실행/중지/관리할 수 있게 해주는 도구임.
#### 주요 활용 상황
...
| 역할 | 설명 |
| --- | --- |
| 서비스 정의 | 컨테이너를 **서비스** 단위로 선언 |
| 네트워크 구성 | 자동으로 custom bridge 네트워크 생성 |
| 볼륨 관리 | 데이터 영속성 보장 |
| 실행 순서 | `depends_on`으로 의존 관계 지정 |
| 환경 설정 | `env`, `secrets`로 환경변수 주입 |
...
> 참고: `docker run -v`는 절대 경로 또는 볼륨명을 사용하는 반면, `docker-compose`의 `volumes`는 **상대 경로(****`./...`****) 사용 가능**함. 기준은 `docker-compose.yaml`이 위치한 디렉토리임.
---
...
- `build.dockerfile` — 사용할 Dockerfile 파일명 지정 (기본값 `Dockerfile`과 다를 때 명시)
...
```
`less`는 결과를 페이지 단위로 보기 좋게 출력하는 유틸리티임. `inspect`로 확인 가능한 정보:
...
| 명령 | 제거 대상 |
| --- | --- |
| `docker compose rm` | 컨테이너만 제거 (네트워크는 그대로) |
...
```
기존 컨테이너를 중지하고 **새로 생성**함. 이미지 변경 없이 yaml 설정만 바뀌었을 때 사용함.
...
```
`ping db`가 성공하면 Compose 내부 DNS가 정상 동작 중임을 의미함. 서비스 이름이 호스트네임으로 자동 등록되기 때문임.
---
### 명령어: 네트워크 점검
#### 네트워크 목록/상세
```bash
...
기본적으로 `docker compose up`은 **현재 디렉토리 이름**을 compose project name으로 사용함. 파일이나 프로젝트 이름을 명시적으로 지정하고 싶을 때 아래 옵션을 씀.
#### f 옵션: 특정 yaml 파일 지정
```bash
...
```
- 같은 이름의 `docker-compose.yaml`이 `app1/`, `app2/` 폴더에 각각 있을 때, `p`로 구분해 다른 프로젝트로 관리할 수 있음
- `docker compose -p myapp up -d` → `myapp`을 project name으로 사용
---
...
```
nginx 설정 파일을 볼륨 마운트로 주입함. 컨테이너 내부의 `default.conf`를 호스트 `nginx.conf`로 덮어씀.
#### 2. nginx.conf
```plain text
server {
    listen80;
...
| 변수 | 의미 | 예시 |
| --- | --- | --- |
| `proxy_pass` | 요청을 전달할 upstream 주소 | `http://backend:8080/` |
| `$host` | 클라이언트가 요청한 도메인 | `example.com` |
| `$remote_addr` | 실제 클라이언트 IP 주소 | `12.45.67.89` |
- `backend`는 Compose 서비스 이름 → Compose 내부 DNS가 자동으로 IP로 해석함
...
---
### command 필드
컨테이너 시작 시 실행할 기본 명령을 yaml에서 재정의하는 필드임. Dockerfile의 `CMD`를 덮어쓰되, `ENTRYPOINT`는 그대로 유지됨.
#### Dockerfile vs docker-compose.yaml 비교
| 상황 | Dockerfile | docker-compose.yaml | 결과 |
| --- | --- | --- | --- |
| CMD만 있을 때 | `CMD ["nginx", "-g", "daemon on;"]` | `command: ["nginx", "-g", "daemon off;"]` | compose의 command가 Dockerfile CMD를 덮어씀 |
| ENTRYPOINT + CMD 있을 때 | `ENTRYPOINT ["docker-entrypoint.sh"]` + `CMD ["nginx", "-g", "daemon on;"]` | `entrypoint: ["docker-entrypoint.sh"]` + `command: ["nginx", "-g", "daemon off;"]` | entrypoint와 command 둘 다 덮어씀 |
- `entrypoint`를 compose에서 재정의하면 Dockerfile의 CMD가 무시되므로, `command`도 함께 재선언해야 함
...
ENV APP_ENV=${APP_ENV}
```
- `ARG`는 빌드 시에만 존재하는 변수 → `docker build --build-arg APP_ENV=prod` 와 동일한 효과
- `ENV`로 변환하면 런타임에도 환경변수로 유지됨
...
| 값 | 재시작 조건 | 주요 용도 |
| --- | --- | --- |
| `no` (기본값) | 재시작 안 함 | 일회성 스크립트, 마이그레이션 도구 |
| `on-failure` | 비정상 종료(exit code ≠ 0) 시만 | 배치 작업, 테스트 컨테이너 |
| `always` | 항상 (정상 종료 포함) | 상시 구동 서비스 |
| `unless-stopped` | `docker stop`으로 명시 중지 시 제외, 나머지는 항상 | stop 이 아닌 종료는 always와 동일 |
- `always`와 `unless-stopped`의 차이: 서버 재부팅 후 **always**는 무조건 재시작, **unless-stopped**는 `docker stop`으로 멈춘 상태였으면 재시작하지 않음
- 실운영 DB나 API 서버는 `unless-stopped`를 선호하는 경우가 많음
---
### Health Check
컨테이너 기동 중 주기적으로 상태를 확인해 `healthy` / `unhealthy`로 표시하는 기능임. 상태에 따른 자동 종료·복구는 지원하지 않으며(Kubernetes의 probe가 그 역할), 주로 `depends_on`의 `condition: service_healthy`와 함께 씀.
```yaml
services:
  db:
    image: postgres:9.6.1
...
| 필드 | 설명 |
| --- | --- |
| `test` | 실행할 헬스 체크 명령 (0 반환 시 healthy)<br>  • 해당 컨테이너 내에서 돌기 때문에 [`localhost`](http://localhost)에서 작동함 |
| `interval` | 체크 주기 |
| `timeout` | 명령 응답 제한 시간 |
| `retries` | 연속 실패 허용 횟수 (초과 시 unhealthy) |
| `start_period` | 컨테이너 초기 기동 유예 시간 (이 시간 동안 실패는 retries에 안 셈) |
---
### 환경변수 주입 방법
...
`docker-compose.yaml`과 같은 디렉토리의 `.env` 파일은 **별도 지정 없이 자동으로 로딩**됨. yaml 내에서 `${변수명}` 형태로 참조 가능함.
```yaml
# .env 파일:
# PORT=3000
...
---
### 네트워크 호출 — 서비스명으로 통신
같은 Compose 네트워크 내 컨테이너 간에는 **서비스 이름이 곧 호스트네임**으로 동작함. IP나 localhost로는 접근 불가함.

- 컨테이너 IP로도 통신은 가능하나, **컨테이너 재생성 시 IP가 바뀔 수 있으므로 서비스명 사용을 권장**함
...
```
- `8080:80`은 `0.0.0.0:8080 → container:80`과 동일
- 0.0.0.0 바인딩이므로 **외부 모든 IP에서 접근 가능**함
...
```
- 외부에서 직접 접근 불가
- 같은 Compose 네트워크 내 다른 서비스는 `db:5432`로 내부 접근 가능
...
- 같은 네트워크의 다른 컨테이너가 접근할 수 있는 포트를 **문서화 목적으로 명시**하는 것에 가까움
...
단순 `depends_on`은 컨테이너 **시작 순서**만 보장하며, 실제 서비스가 준비됐는지는 보장하지 않음. `condition: service_healthy`를 사용하면 healthcheck가 통과된 후에야 다음 서비스를 기동함.
```yaml
services:
  db:
    image: postgres:9.6.1
