---
title: "[7/29] 스마트 데이터 이해 및 활용_Day3_실습"
notion_page_id: "3ac1d84b-f68e-8102-ba1b-f149fdd185b9"
source_url: "https://app.notion.com/p/3ac1d84bf68e8102ba1bf149fdd185b9"
synced_at: "2026-07-30T00:09:33+09:00"
content_sha256: "32b9646bbe54406c23fe991f3fc0723d253864f12160606617658dadffe4f1d5"
tags: [notion, skala, learning, database, sql, practice]
---

# [7/29] 스마트 데이터 이해 및 활용 — Day3 — 실습

[[notion/SKALA/index|SKALA 학습 노트]]

[[notion/SKALA/7-29 스마트 데이터 이해 및 활용_Day3/7-29 스마트 데이터 이해 및 활용_Day3 핵심 정리|Day3 핵심 정리]]

## 학습 범위

이 노트는 Notion 원문의 Day3 데이터베이스·SQL 학습 내용을 보존한다. 임시 서명 이미지 URL은 장기 보존 근거로 사용하지 않아 제외했다.

> 원문: [Notion 페이지](https://app.notion.com/p/3ac1d84bf68e8102ba1bf149fdd185b9) (2026-07-30 확인)
...
---
## 전체 요약
| 문제 | 주제 | 핵심 인덱스 / 기법 | Execution Time |
| --- | --- | --- | --- |
| 1 | PK 단건 조회 | (튜닝 불필요, PK 인덱스) | 0.056 ms 유지 |
| 2 | 표현식 인덱스 | lower(email) | 20.343 → 0.056 ms |
| 3 | pg_trgm GIN | gin_trgm_ops | 8.052 → 1.635 ms |
| 4 | 부분 정렬 인덱스 | salary DESC + INCLUDE + WHERE ACTIVE | 7.703 → 0.104 ms |
| 5 | OR vs IN | branch_code | 12.682 → 0.83\~0.86 ms |
| 6 | SARGability | hire_date 범위 재작성 | 4.931 → 1.439 ms (B) |
| 7 | 복합 인덱스 | (department_id, job_code, salary DESC) | 7.470 → 0.071 ms |
| 8 | 커버링 인덱스 | employee_no INCLUDE + VACUUM | 21.839 → 0.126 ms |
| 9 | 조인 loops | (employee_id, work_date) INCLUDE + 부분 인덱스 | 25.814 → 8.015 ms |
| 10 | NOT IN NULL 함정 | NOT EXISTS + 부분 인덱스 | 정확성 0 → 14000 |
## 배운 핵심 포인트
