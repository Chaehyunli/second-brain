---
title: "[8/14] Java, SpringBoot, Rest API 구현_Day5_핵심 정리"
notion_page_id: "3bc1d84b-f68e-8044-b4b4-e5e7f4d7bb79"
source_url: "https://app.notion.com/p/3bc1d84bf68e8044b4b4e5e7f4d7bb79"
synced_at: "2026-08-15T00:09:24+09:00"
content_sha256: "e2ba8ae4789243d8c5e9de08ba0cf0d04d6ba43b7570198d8bc55035f9e94fd7"
tags: [notion, skala, learning, java, spring-boot, jpa, rest-api]
---

# [8/14] Java, SpringBoot, Rest API 구현_Day5_핵심 정리

[[notion/SKALA/index|SKALA 학습 노트]]

[[notion/SKALA/8-13 Java, SpringBoot, Rest API 구현_Day4/[8-13] Java, SpringBoot, Rest API 구현_Day4_핵심 정리|Day4 — Lombok·Spring Boot·REST API]]

> 원문: [Notion 페이지](https://app.notion.com/p/3bc1d84bf68e8044b4b4e5e7f4d7bb79) (2026-08-15 KST 확인)
...
| Object (객체) | Relational Table (테이블) |
| --- | --- |
| User (클래스) | users (테이블) |
| id (Long, Primary Key) | id (BIGINT, PRIMARY KEY, AUTO_INCREMENT) |
| name (String) | name (VARCHAR(100), NOT NULL) |
| email (String) | email (VARCHAR(150), UNIQUE, NOT NULL) |
...
| 구분 | JPA | JDBC |
| --- | --- | --- |
| 목적 | 자바 객체와 DB 테이블 매핑(ORM) | SQL을 직접 실행하여 DB와 연동 |
| 코딩 방식 | 객체지향(Entity/Repository/Service 계층), SQL 최소화 | SQL 직접 작성, Connection/Statement/ResultSet 사용 |
| 자동화 수준 | CRUD·트랜잭션·매핑·변경감지 등 많은 부분 자동화 | 모든 DB 작업을 개발자가 직접 처리 |
| SQL 작성 | 메서드 자동 생성, `@Query`를 통한 쿼리 작성 가능 | 모든 쿼리를 직접 작성 |
| 트랜잭션 | `@Transactional`로 선언적 관리 | Connection 객체로 직접 트랜잭션 제어 |
| 예외 처리 | `DataAccessException` 등 추상화된 예외 사용 | `SQLException` 사용, 직접 예외 처리 필요 |
| 데이터 매핑 | 객체(Entity)와 테이블 자동 매핑 | ResultSet에서 직접 필드 추출, 수동 매핑 |
| 학습 난이도 | ORM·엔티티·영속성 컨텍스트 등 개념 필요 | JDBC와 SQL만 알면 바로 사용 가능 |
| 성능 최적화 | 캐시·배치 처리 등 별도 튜닝 필요 | SQL 및 JDBC 커넥션 풀 등 직접 튜닝 |
| 대표 사용 예 | Spring Data JPA, Hibernate | JdbcTemplate, MyBatis, 순수 JDBC 코드 |
| 커넥션 관리 | 스프링이 자동 관리, 커넥션 풀 연동 | 직접 관리(try-finally 등으로 close 필수) |
---
### Database 연결 설정 (application.yml)
Spring Boot에서 JPA 및 데이터베이스 연결 정보는 `application.yml`에 설정함.
...
| 어노테이션 | 목적/용도 |
| --- | --- |
| `@Entity` | 클래스를 JPA가 관리하는 엔티티로 지정. 이 클래스를 테이블에 매핑해 CRUD 처리 가능하게 함 |
| `@Table` | 엔티티와 매핑할 테이블의 이름·스키마 지정. 클래스명과 실제 테이블명이 다를 때 사용 |
| `@Id` | 기본키(Primary Key) 필드 지정. 각 엔티티마다 식별자가 필요하며 객체의 고유성을 보장 |
| `@GeneratedValue` | 기본키 값을 자동 생성하는 전략 설정. auto_increment·시퀀스 등 활용 |
| `@Column` | 클래스 필드와 DB 컬럼을 세부적으로 매핑. 컬럼명·길이·null 허용·유니크 등 제약조건 지정 |
| `@Enumerated` | Enum 타입 필드를 DB에 저장할 방식 지정. ORDINAL(숫자) 또는 STRING(문자열) 선택 |
| `@Lob` | 대용량 데이터(텍스트/이미지 등) 저장. String → CLOB, byte\[\] → BLOB으로 매핑 |
| `@Transient` | DB에 저장하지 않는 필드 선언. 계산된 값·임시 데이터 등에 사용 |
| `@{One/Many}To{One/Many}` | 엔티티 간 관계 명시. 외래키와 매핑하며 1:1·1:N·N:1·N:N 등 지원 |
| `@JoinColumn` | 관계 매핑 시 외래키 컬럼을 직접 지정 |
| `@Embedded` | 별도 클래스를 필드로 포함하여 컬럼을 그룹화해 재사용 |
| `@Embeddable` | `@Embedded`로 포함될 클래스에 선언. 여러 엔티티에서 재사용 가능 |
---
### JPA 어노테이션 - @Entity
해당 클래스를 JPA가 관리하는 엔티티(테이블)로 지정하며, 자바 클래스와 데이터베이스 테이블을 1:1로 매핑하는 역할을 함. `@Entity`가 선언된 클래스는 JPA의 CRUD 대상이 되고, Persistence Context 안에서 생명주기가 관리됨.
유의 사항.
- `@Entity`가 붙은 클래스는 반드시 기본 생성자(파라미터 없는 생성자)가 필요함
- `@Entity`만 선언하면 클래스 이름이 그대로 테이블명으로 매핑됨
- 실제 운영에서는 테이블명과 클래스명이 다를 수 있으므로, 보통 `@Table`을 함께 사용해 테이블명을 명시함
```java
// Stock.java
...
---
### JPA 어노테이션 - @Table
`@Entity`로 지정한 클래스가 어느 DB 테이블과 매핑될지를 명시함. 테이블 이름이 클래스명과 다를 때 사용하거나, 스키마 지정 등 추가 설정이 필요할 때 사용함. 생략 가능하며, 생략 시 클래스명이 테이블명으로 사용됨.
| 속성 | 설명 | 예시 |
| --- | --- | --- |
| `name` | 매핑할 DB 테이블 이름 지정 | `@Table(name = "stocks")` |
| `schema` | 사용할 DB 스키마 지정 (DBMS가 지원할 때만) | `@Table(name = "stocks", schema = "skala")` |
```java
...
---
### JPA 어노테이션 - @Id
엔티티의 기본키(Primary Key) 필드를 지정함. JPA는 모든 엔티티에 반드시 하나 이상의 `@Id`가 필요함. `@Id`로 지정된 필드를 통해 엔티티 객체의 고유성이 보장됨.
...
---
### JPA 어노테이션 - @Id (복합키)
두 개 이상의 컬럼 조합이 고유해야 하는 경우에 복합키를 사용함. `@Embeddable` 클래스로 복합키 타입을 정의하고, 엔티티에서 `@EmbeddedId`로 참조하는 방식임.
```java
...
기본키(PK) 값을 자동으로 생성하는 전략을 지정할 때 사용하며, 반드시 `@Id`와 함께 사용함. DB의 auto_increment, 시퀀스, 별도 테이블 등 다양한 자동 생성 방식을 지원함.
주요 속성.
...
| strategy | 동작 |
| --- | --- |
| `AUTO` | DB에 맞는 전략을 JPA가 알아서 선택 (기본값) |
| `IDENTITY` | INSERT 시점에 ID 생성. MySQL, MariaDB, SQL Server 등에서 사용 |
| `SEQUENCE` | PK를 미리 뽑아서 INSERT 시 사용. Oracle, PostgreSQL 등에서 사용 |
```java
...
| 속성 | 설명 | 예시 |
| --- | --- | --- |
| `name` | 컬럼명 지정 | `@Column(name = "user_name")` |
| `nullable` | null 허용 여부 (기본값: true) | `@Column(nullable = false)` |
| `length` | 문자 길이(문자열 필드에만 사용) | `@Column(length = 50)` |
| `unique` | 유니크 제약조건 부여(단일 컬럼) | `@Column(unique = true)` |
| `updatable` | UPDATE 쿼리에서 수정 가능 여부 | `@Column(updatable = false)` |
| `precision, scale` | 소수점 숫자 자리수 지정(숫자형만) | `@Column(precision = 10, scale = 2)` |
종합 사용 예시.
```java
// User.java
@Entity
...
```
숫자 자릿수 지정 예시 — `precision`은 전체 자릿수, `scale`은 소수점 이하 자릿수를 의미함.
```java
...
```
실무 참고 사항 — DTO 객체를 사용하여 내부에서 임시 저장하고, 실제 데이터베이스 저장 시에만 Entity를 사용하는 구조로 설계하면 `@Transient`가 필요한 상황 자체가 줄어듦.
---
### JPA 어노테이션 - @Transient
엔티티 필드 중에서 DB 컬럼과 매핑하지 않을 필드에 사용함. 해당 필드는 DB 테이블에 컬럼으로 생성되지 않으며, JPA의 영속화(저장/조회) 대상이 아님을 명시함.
계산값, 임시 데이터, 화면 출력 전용 속성, 비밀번호 확인 필드 등에 주로 활용함.
...
---
### JPA Repository 정의
JPA에서 Repository는 데이터베이스와 직접적인 상호작용을 담당하는 인터페이스임. Spring Data JPA가 제공하는 `JpaRepository`를 상속하면 기본적인 CRUD(Create, Read, Update, Delete) 기능을 별도의 SQL 코드 없이 사용할 수 있음.
```java
...
```
내부 동작 원리 — `JpaRepository`가 구현한 메서드(`findAll`, `findById`, `save`, `delete`)를 상속받아 제공하는 구조임. Spring이 `UserRepository`를 상속받은 Proxy를 동적으로 생성해서 Bean으로 등록함.
---
...
| 인터페이스 | 설명 |
| --- | --- |
| Repository | 가장 상위의 기본 인터페이스. 아무런 메서드도 정의되어 있지 않은 마커(Marker) 인터페이스 |
| CrudRepository | Repository를 상속받으며, 데이터의 CRUD 기능을 위한 기본 메서드들을 제공 |
| PagingAndSortingRepository | CrudRepository를 상속받으며, 페이징과 정렬 기능을 위한 메서드를 추가로 제공 |
| JpaRepository | PagingAndSortingRepository를 상속받으며, JPA에 특화된 기능들을 추가로 제공. 일반적으로 가장 많이 사용되는 인터페이스 |
---
### JPA Repository 기본 Method
`JpaRepository` 선언 시 자동으로 구성되는 기본 메서드 목록.
| 메서드 | 설명 | SQL 동작 |
| --- | --- | --- |
| `save(S entity)` | 엔티티 저장 (INSERT or UPDATE) | INSERT 또는 UPDATE |
| `saveAll(Iterable<S> entities)` | 여러 엔티티를 한 번에 저장 | BULK INSERT 또는 UPDATE |
| `findById(ID id)` | ID(PK)로 엔티티 조회 | `SELECT * WHERE id = ?` |
| `existsById(ID id)` | 특정 ID의 엔티티 존재 여부 확인 | `SELECT COUNT(*) WHERE id = ?` |
| `findAll()` | 모든 엔티티 조회 (Page 단위 조회 가능) | `SELECT * FROM table` |
| `findAllById(Iterable<ID> ids)` | 여러 ID에 해당하는 엔티티 조회 | `SELECT * WHERE id IN (?)` |
| `count()` | 엔티티 개수 반환 | `SELECT COUNT(*) FROM table` |
| `deleteById(ID id)` | 특정 ID의 엔티티 삭제 | `DELETE FROM table WHERE id = ?` |
| `delete(T entity)` | 특정 엔티티 삭제 | `DELETE FROM table WHERE id = ?` |
| `deleteAllById(Iterable<ID> ids)` | 여러 ID의 엔티티 삭제 | `DELETE FROM table WHERE id IN (?)` |
| `deleteAll(Iterable<? extends T> entities)` | 여러 엔티티 삭제 | `DELETE FROM table WHERE id IN (?)` |
| `deleteAll()` | 모든 엔티티 삭제 | `DELETE FROM table` |
| `flush()` | 현재 영속성 컨텍스트의 변경 내용을 즉시 반영 | FLUSH |
| `saveAndFlush(S entity)` | 저장 후 즉시 데이터베이스에 반영 | INSERT/UPDATE + FLUSH |
| `deleteAllInBatch()` | 전체 데이터 삭제 (개별 DELETE 없이 한 번에) | `DELETE FROM table` |
| `deleteAllByIdInBatch(Iterable<ID> ids)` | 여러 개의 데이터를 한 번에 삭제 | `DELETE FROM table WHERE id IN (?)` |
...
| 메서드 시그니처 | 설명 | 리턴값 |
| --- | --- | --- |
| `<S extends T> S save(S entity)` | 엔티티 저장 또는 병합(수정) | S |
| `<S extends T> List<S> saveAll(Iterable<S> entities)` | 여러 엔티티 저장/수정 | `List<S>` |
| `<S extends T> S saveAndFlush(S entity)` | 엔티티 저장/수정 후 즉시 flush | S |
| `void flush()` | 영속성 컨텍스트 변경 내용을 DB에 즉시 반영 | void |
...
- 단점 : 조인, 복잡한 논리(AND/OR 혼합), 그룹/집계, 복잡한 동적 쿼리에는 적합하지 않음. 이름이 너무 길어지면 `@Query`로 전환을 고려함
...
| 키워드 | 설명 | 예시 |
| --- | --- | --- |
| `findBy` | 조회 | `findByEmail(String email)` |
| `readBy` | 조회 (find와 동일) | `readByEmail(String email)` |
| `queryBy` | 조회 (find와 동일) | `queryByEmail(String email)` |
| `getBy` | 조회 (find와 동일) | `getByEmail(String email)` |
| `countBy` | 카운트 조회 | `countByStatus(String status)` |
| `existsBy` | 존재 여부 (true, false) | `existsByEmail(String email)` |
| `deleteBy` | 엔티티 삭제 | `deleteByEmail(String email)` |
| `removeBy` | 엔티티 삭제 | `removeByEmail(String email)` |
리턴 타입은 선언에 따라 자동 구성됨. 단건 조회 시 `Optional<T>` 사용을 권장하며, 여러 건 리턴 시에는 `Optional`을 사용하지 않음.
---
### JPA 메서드를 위한 사전 정의 키워드 (2/2) — 조건 연산자
| 키워드 | 설명 | 예시 |
| --- | --- | --- |
| `And` | AND로 결합 | `findByNameAndEmail(String name, String email)` |
| `Or` | OR로 결합 | `findByNameOrEmail(String name, String email)` |
| `OrderBy` | 정렬 기준 추가 | `findByNameOrderByEmailAsc(String name)` |
| `Between` | 두 값 사이의 범위 검색 | `findByAgeBetween(int min, int max)` |
| `GreaterThan` | 특정 값보다 큰 값 검색 | `findByAgeGreaterThan(int age)` |
| `LessThan` | 특정 값보다 작은 값 검색 | `findByAgeLessThan(int age)` |
| `Like` | 특정 문자열 포함 검색 (와일드카드 직접 포함) | `findByNameLike("%홍%")` |
| `In` | 여러 개 중 하나를 포함하는 조회 | `findByStatusIn(List<String> statuses)` |
---
### \[참고\] JPA 사용자 쿼리 메서드 전체 키워드 정리
| 구분 | 키워드 | 의미/동작 | 예시 메서드명 |
| --- | --- | --- | --- |
| 조회 | `find, read, get, query, search, stream, count, exists, delete, remove` | 조회·개수·존재 여부·삭제 등 | `findByName`, `countByAge`, `existsByEmail` |
| 조건 | `By` | 뒤에 오는 조건 필드를 지정 | `findByName`, `findByAge` |
|  | `And, Or` | 조건 연결(AND/OR) | `findByNameAndAge`, `findByNameOrEmail` |
|  | `Is, Equals` | 같음(=) | `findByNameIs`, `findByNameEquals` |
|  | `Between` | 범위(사이) | `findByAgeBetween` |
|  | `LessThan, Before` | 미만(\<) | `findByAgeLessThan` |
|  | `GreaterThan, After` | 초과(\>) | `findByAgeGreaterThan` |
|  | `LessThanEqual` | 이하(\<=) | `findByAgeLessThanEqual` |
|  | `GreaterThanEqual` | 이상(\>=) | `findByAgeGreaterThanEqual` |
|  | `IsNull, IsNotNull` | 널/널이 아님 | `findByEmailIsNull`, `findByEmailIsNotNull` |
|  | `In, NotIn` | 컬렉션 내 포함/불포함 | `findByStatusIn`, `findByAgeNotIn` |
|  | `StartingWith` | 문자열로 시작하는(`Like 'xxx%'`) | `findByNameStartingWith` |
|  | `EndingWith` | 문자열로 끝나는(`Like '%xxx'`) | `findByNameEndingWith` |
|  | `Containing, Contains` | 포함(`Like '%xxx%'`) | `findByNameContaining` |
|  | `Like, NotLike` | Like, Not Like | `findByNameLike`, `findByNameNotLike` |
|  | `True, False` | 불리언 값 | `findByActiveTrue` |
| 정렬 | `OrderBy` | 정렬 조건 | `findByNameOrderByAgeDesc` |
| 페이징 | `Pageable, Slice` | 페이징 파라미터 사용 | `findByName(String name, Pageable pageable)` |
---
...
```
→ `column` 명이 아닌 객체의 이름 `user u`를 기준으로 작성
---
...
| 분류 | 함수명 | 설명/용도 | 예시 코드 |
| --- | --- | --- | --- |
| 집계함수 | `COUNT` | 행 개수 집계 | `select count(u) from User u` |
|  | `SUM` | 합계 | `select sum(o.amount) from Order o` |
|  | `AVG` | 평균 | `select avg(u.age) from User u` |
|  | `MAX` | 최대값 | `select max(u.age) from User u` |
|  | `MIN` | 최소값 | `select min(u.age) from User u` |
| 문자열 | `CONCAT` | 문자열 연결 | `select concat(u.firstName, ' ', u.lastName) from User u` |
|  | `SUBSTRING` | 문자열 자르기 | `select substring(u.name, 1, 3) from User u` |
|  | `LENGTH` | 문자열 길이 | `select length(u.name) from User u` |
|  | `LOWER` | 소문자 변환 | `select lower(u.name) from User u` |
|  | `UPPER` | 대문자 변환 | `select upper(u.name) from User u` |
|  | `TRIM` | 앞뒤 공백 제거 | `select trim(u.name) from User u` |
| 날짜/시간 | `CURRENT_DATE` | 현재 날짜 | `select current_date from User u` |
|  | `CURRENT_TIME` | 현재 시간 | `select current_time from User u` |
|  | `CURRENT_TIMESTAMP` | 현재 날짜/시간 | `select current_timestamp from User u` |
---
### 영속성 컨텍스트 (Persistence Context) 상태 관리
Entity를 저장하거나 검색하는 경우에는 영속화(Persistence Context) 상태로 전환됨. 엔티티는 생명주기에 따라 네 가지 상태를 가짐.
| 상태 | 명칭 | 설명 |
| --- | --- | --- |
| 비영속 | new / transient | 순수 자바 객체 상태. 영속성 컨텍스트와 무관함 |
| 영속 | managed | 영속성 컨텍스트 메모리에 등록되고 관리되는 상태 |
| 준영속 | detached | 생성된 Entity 객체가 영속성 컨텍스트에서 떨어져 나가 더 이상 동기화되지 않는 상태 |
| 삭제 | removed | 삭제된 상태. 트랜잭션 커밋 시 DELETE 쿼리 실행 |
상태 전이 흐름.
![]()
각 상태별 코드 예시.
```java
...
| 메서드 | 상태 전이 | 설명 |
| --- | --- | --- |
| `persist()` | New → Managed | 비영속 상태의 객체를 영속성 컨텍스트에 등록하여 관리 상태로 바꿈 |
| `find()` / JPQL | DB → Managed | DB에서 데이터를 조회해 올 때, JPA는 가져온 데이터를 바로 Managed 상태로 만들어 영속성 컨텍스트에 올림 |
| `detach()` / `clear()` / `close()` | Managed → Detached | 영속 상태의 엔티티를 관리 대상에서 제외함. `clear()`는 컨텍스트 전체 초기화, `close()`는 종료 |
| `merge()` | Detached → Managed | 준영속 상태의 객체를 다시 영속성 컨텍스트 관리 상태로 복구시킴 |
| `remove()` | Managed → Removed | 영속 상태의 객체를 삭제 대기 상태로 만듦 |
| `flush()` | Managed/Removed → DB | 영속성 컨텍스트에서 변경된 내용이나 삭제 요청을 DB로 쿼리(INSERT/UPDATE/DELETE)를 날려 동기화함 |
...
---
### 관계 매핑 : ManyToOne
여러 Member가 하나의 Team에 속하는 N:1 관계임. FK가 있는 쪽(MEMBER 테이블)이 연관관계의 주인이 되며, `@ManyToOne`과 `@JoinColumn`을 함께 사용함.
```java
...
---
#### 관계 매핑 : OneToMany
하나의 Team이 여러 Member를 가지는 1:N 관계임. `@OneToMany`는 FK가 없는 쪽(Team)에 선언하며, `mappedBy` 속성으로 연관관계의 주인 필드명을 지정함. DB에서 FK는 여전히 MEMBER 테이블에 존재함.
```java
...
```
`mappedBy = "team"`은 "Member 클래스의 `team` 필드가 이 관계의 주인"임을 의미함. 실제 FK 관리(INSERT/UPDATE)는 `@ManyToOne`을 가진 Member 쪽에서 이루어짐.
→ `@OneToMany(mappedBy = "team", fetch = FetchType.LAZY)` 처럼 사용시, 지연 로딩이 됨(`Team`을 가저올 때, `Member`를 모두 다 가져오는 것 비효율적)
---
#### JPA 어노테이션 - 관계 매핑
객체 간의 관계를 데이터베이스 테이블의 외래키(FK)와 연결하는 방법을 정의함. 다중성(1:1, 1:N, N:1, N:M)과 방향성(단방향, 양방향)을 조합하여 설정함.
| 어노테이션 | 관계 | 대표 예시 |
| --- | --- | --- |
| `@ManyToOne` | N:1 | 여러 거래 → 하나의 주식 |
| `@OneToMany` | 1:N | 하나의 사용자 → 여러 거래 |
| `@OneToOne` | 1:1 | 유저 ↔ 프로필 |
| `@ManyToMany` | N:M | 유저 ↔ 관심종목(주식) |
| `@JoinColumn` | 외래키 지정 | 거래에 user_id, ticker 등 |
| `@JoinTable` | 중간테이블 | user_watchlist(user_id + ticker) |
연관관계의 주인은 FK가 있는 쪽이 됨.
- `@OneToMany(mappedBy = "user")`라면 추가/삭제/변경의 주체는 `@ManyToOne`을 가진 Entity임
- SQL은 항상 주체 쪽 변경을 기준으로 생성됨
---
#### FetchType.LAZY vs EAGER
연관관계 매핑 시 연관된 엔티티를 언제 DB에서 조회할지를 결정하는 전략임.
- `@OneToMany`의 기본값 : `FetchType.LAZY`
- `@ManyToOne`의 기본값 : `FetchType.EAGER`
| FetchType | 동작 | 특징 |
| --- | --- | --- |
| LAZY (지연 로딩) | 실제로 접근하는 순간에 SELECT 쿼리 발생 | 성능에 유리. 프록시 객체로 우선 채움 |
| EAGER (즉시 로딩) | 엔티티 조회 시 연관 엔티티를 즉시 JOIN해서 함께 가져옴 | 불필요한 쿼리가 발생할 수 있음 |
...
```
LAZY인 경우 `Product`를 조회할 때 `user`는 즉시 쿼리로 가져오지 않고 Proxy로 채움. `getName()`처럼 실제 필드에 접근하는 순간 별도 SELECT가 나가며 초기화됨.
---
### EntityManager
JPA에서 엔티티(객체)를 DB와 동기화하기 위해 사용하는 핵심 관리 객체임. 엔티티의 생성·조회·수정·삭제(CRUD)와 트랜잭션, 영속성 컨텍스트 관리 등을 수행하며, JPA가 DB와 대화하는 모든 기능의 중심임.
![]()
웹 애플리케이션이 구동되는 시점에 `EntityManagerFactory`를 생성하여 보유하고 있으며, 사용자의 요청이 있을 때 `EntityManager`를 생성하여 커넥션 풀(Connection Pool)을 사용해서 DB를 핸들링함. 각 요청(스레드)마다 별도의 EntityManager 인스턴스가 할당됨.
---
### EntityManager의 주요 메서드
Spring Boot + JPA(Hibernate)에서 가장 자주 쓰는 메서드는 `find`, `persist`, `merge`, `remove`, `flush`, `clear`임.
| 분류 | 메서드 | 설명 |
| --- | --- | --- |
| 조회(Read) | `find(Class<T> entityClass, Object primaryKey)` | PK 기반 즉시 조회 (1차 캐시 먼저 확인) |
|  | `getReference()` | 실제 사용 시까지 프록시 – 지연 로딩 |
|  | `createQuery(String qlString)` | JPQL 기반 조회 |
|  | `createNamedQuery(String name)` | `@NamedQuery` 조회 |
|  | `createNativeQuery(String sqlString)` | 네이티브 SQL 조회 |
| 저장/변경(Write) | `persist(Object entity)` | 신규 엔티티 영속화 (INSERT 예정) |
|  | `merge(Object entity)` | Detached 객체(일반객체)를 다시 영속상태로 병합 |
|  | `remove(Object entity)` | 영속 엔티티 제거 (DELETE 예정) |
|  | `flush()` | 영속성 컨텍스트 변경 내용을 DB에 즉시 반영 |
|  | `clear()` | 영속성 컨텍스트 초기화 (1차 캐시 제거) |
| 트랜잭션/상태 관리 | `contains(Object entity)` | 해당 객체가 영속 상태인지 여부 |
|  | `detach(Object entity)` | 특정 엔티티만 영속성 컨텍스트에서 분리 |
|  | `setFlushMode(FlushModeType)` | flush 시점 제어 (COMMIT, AUTO) |
| 동시성 관련 | `lock(Object entity, LockModeType lockMode)` | 낙관/비관적 Lock 적용 |
| 저장소 접근 | `unwrap(Class<T>)` | 하위 구현체(ex: Hibernate Session) 얻기 |
|  | `getEntityManagerFactory()` | 연결된 EMF 조회 |
| 기타 | `getTransaction()` | 리소스 로컬 트랜잭션 획득 (JTA 환경에서는 사용 X) |
---
### EntityManager vs JpaRepository
`JpaRepository`는 `EntityManager`를 더 쓰기 쉽게 감싸놓은 상위 수준의 추상화 도구임. 내부적으로 `JpaRepository`는 결국 `EntityManager`를 사용해 동작함.
| 구분 | EntityManager (JPA 기본) | JpaRepository (Spring Data JPA) |
| --- | --- | --- |
| 정의 | JPA 표준 스펙의 핵심 인터페이스 | Spring이 JPA를 쉽게 쓰도록 만든 추상화 라이브러리 |
| 작동 레벨 | 로우 레벨 (데이터베이스와 직접 소통) | 하이 레벨 (개발자 친화적 인터페이스) |
| 구현 방식 | 개발자가 직접 메서드를 호출하고 쿼리 작성 | 인터페이스 선언만으로 Spring이 알아서 구현체 생성 |
| 코드 양 | 상대적으로 많음 (보일러플레이트 코드 발생) | 매우 적음 (메서드 이름만으로 쿼리 생성) |
---
### 영속성 컨텍스트 (Persistence Context)
`EntityManager`는 하나의 트랜잭션별로 Persistence Context를 생성·관리함. `@Transactional`을 사용하여 Persistence Context를 활성화함.
![]()
영속성 컨텍스트는 엔티티 객체를 1차 캐시로 보관하여, 해당 엔티티와 데이터베이스 레코드를 동기화하기 위해 상태를 추적·관리하는 메모리 공간임. `findById`, `findAll` 등으로 DB에서 조회한 엔티티는 Persistence Context에 올라와 관리되며, 트랜잭션 커밋 시 `flush`를 통해 DB에 반영됨.
---
### 영속성 컨텍스트 (Persistence Context) 역할
JPA의 작업장을 모아두는 1차 캐시 + 변경 감지 시스템임.
| 기능 | 설명 |
| --- | --- |
| 1차 캐시 | 같은 엔티티를 다시 조회하면 DB hit 없이 캐시에서 반환 |
| 엔티티 동일성 보장 | 같은 PK 조회 시 항상 동일 객체(`==` true) |
| 변경 감지(Dirty Checking) | 엔티티 변경을 자동 감지 → UPDATE SQL 자동 실행 |
| 지연 로딩(Lazy Loading) | 필요할 때만 연관 엔티티 로딩 |
| 쓰기 지연(Write-behind) | SQL을 모아서 트랜잭션 끝에서 한 번에 flush |
...
`JpaRepository`는 인터페이스만 선언해도 자동으로 구현체가 생성되는데, Proxy pattern으로 `EntityManager`를 생성하여 호출하는 구조로 지원함. `userRepository.save(user)` 호출 시 내부적으로 다음 흐름으로 처리됨.
```plain text
...
---
### EntityManager 사용 예시
모든 메서드에 `@Transactional`을 설정하여 사용함. Spring에서는 `@PersistenceContext`로 주입받은 `em`은 실제 EntityManager가 아니라 Proxy EntityManager임. 요청마다 트랜잭션에 맞는 실제 EntityManager를 대신 호출해주는 구조임.
```java
// UserService.java
@Service
...
| 원칙 | 설명 |
| --- | --- |
| Atomicity(원자성) | 모두 반영되거나 모두 반영되지 않아야 함 (all or nothing) |
| Consistency(일관성) | 트랜잭션 전후로 데이터가 일관된 상태 유지해야 함 |
| Isolation(격리성) | 여러 트랜잭션이 동시에 실행돼도 서로 영향 없어야 함 |
| Durability(지속성) | 커밋된 결과는 영구적으로 저장되어야 함 |
...
`org.springframework.transaction.annotation.Transactional`을 사용해야 함.
```java
// UserService.java
@Service
...
DB 변경이 있는 오퍼레이션을 수행하는 서비스 메서드에는 `@Transactional`을 꼭 사용하는 것이 권장됨. 너무 남발하면 트랜잭션 범위가 넓어져 DB lock·동시성 저하 등 성능 문제가 생길 수 있음.
| 케이스 | 트랜잭션 적용 방법 |
| --- | --- |
| 데이터 변경 (insert/update/delete) | 반드시 `@Transactional` 사용 |
| 단순 조회 (select) | `@Transactional(readOnly = true)` |
| 외부 API/파일 등 장시간 IO 포함 | 트랜잭션 범위를 DB 오퍼레이션만 포함하도록 최대한 좁게 유지 |
`@Transactional`을 붙이지 않으면 `save` 시 commit이 일어나지 않나? — `JpaRepository`를 사용하는 경우 기본적으로 `save` 호출 시 `@Transactional`을 method에 붙이고 있음. 하지만 원자성 및 성능 저하/비효율이 발생하므로, 서비스 레이어에서 직접 트랜잭션을 관리하는 것이 올바른 방식임.
---
### \[참고\] @Transactional 설정 범위 예시
외부 API 호출·파일 IO 등 장시간 작업이 포함된 메서드 전체에 `@Transactional`을 걸면 트랜잭션 범위가 너무 넓어짐. DB 작업만 별도 메서드로 분리하고 그 메서드에만 `@Transactional`을 적용하는 것이 올바른 방식임.
```java
// 나쁜 예 - 트랜잭션 범위가 너무 넓음
@Service
...
```
트랜잭션은 AOP Proxy를 통해 호출되므로, 내부 사용 함수지만 `public`으로 선언해야 함.
---
...
`@Transactional` 동작 원리.
- 스프링에서 서비스 클래스의 메서드에 `@Transactional`을 붙이면, 스프링 컨테이너는 해당 서비스 객체 대신 트랜잭션 기능이 포함된 프록시(Proxy) 객체를 생성하여 빈(Bean)으로 등록함. 서비스 메서드를 호출하면 실제로는 이 프록시 객체의 메서드가 먼저 호출됨
...
| 옵션명 | 설명 |
| --- | --- |
| REQUIRED (기본값) | 기존 트랜잭션이 있으면 참여, 없으면 새로 생성. 대부분의 서비스 로직에 사용 |
| REQUIRES_NEW | 항상 새로운 트랜잭션 생성. 기존 트랜잭션은 일시 중단(suspend), 별도의 영속성 관리. A 트랜잭션의 성공/실패와 무관하게 B 작업을 반드시 독립적으로 커밋시키고 싶을 때 사용. 로그 기록·감사(audit) 등에 활용 |
---
### 트랜잭션 전파 오류 - Self Invocation 문제
하나의 클래스 내에서 `@Transactional`이 붙은 다른 메서드를 호출하면 트랜잭션 전파 설정이 적용되지 않음. 이는 프록시를 거치지 않고 실제 객체의 내부 메서드를 직접 호출하기 때문임.
```java
...
```
`InnerService`는 `OuterService` Bean 생성 시점에 AOP Proxy로 Wrapping된 후 `InnerService`가 DI되어 개별 트랜잭션으로 처리됨.
---
### \[참고\] 트랜잭션 예외와 롤백 규칙
`@Transactional`은 기본적으로 `RuntimeException`(Unchecked Exception)과 `Error`가 발생했을 때만 롤백함.
![]()
- Checked Exception(`Exception`을 상속받는)을 `try-catch`로 잡고 아무것도 하지 않으면, 트랜잭션은 정상적으로 커밋되어 데이터 정합성 문제가 발생할 수 있음
...
| 문제 유형 | 설명 | 발생 여부 |
| --- | --- | --- |
| Lost Update | 두 사용자가 동시에 같은 User의 point를 증가시키는 경우. 두 Thread가 동시에 값을 읽고, 각각 업데이트 | O — 충돌 감지 불가 |
| Dirty Read | Isolation이 필수적으로 지원하기 때문에 발생하지 않음 | X — 기본 ReadCommitted로 방지됨 |
| 반복 읽기 불일치 (Non-repeatable Read) | 한 트랜잭션에서 같은 엔티티를 두 번 읽는 사이에 다른 트랜잭션이 데이터를 바꾼 경우 | O — JPA 1차 캐시와 DB 상태 불일치 |
| 환영 읽기 (Phantom Read) | 트랜잭션이 동일한 조건으로 동일한 쿼리를 두 번 실행했을 때, 두 번째 조회에서 새로운 Row가 추가되어 나타나는 현상 | O — Lock 없으면 방지 불가 |
| 경쟁 조건 (Race Condition) | 동시성 경쟁으로 비즈니스 로직이 잘못 처리. 예) 두 스레드 모두 재고 감소 실행 | O — 재고 감소/포인트 증가 등에서 치명적 |
---
...
```
`@Version`을 Entity에 설정하고 `@Lock`을 `@Transactional`에서 사용하지 않으면 자동으로 낙관적 Lock으로 동작함.
→ 최근 트렌드는 낙관적 락임
---
### 낙관적 락 (Optimistic Lock) Retry
낙관적 Lock은 `@Transactional`이 종료되면서 commit 시점에 다른 곳에서 업데이트를 먼저 했으면 Failure가 발생함. 재 처리를 위해서는 `@Retryable`을 사용해서 재처리함.
...
| 어노테이션 | 적용 위치 | 주요 용도/설명 |
| --- | --- | --- |
| `@Tag` | 클래스(Controller) | API 그룹 및 설명. Swagger UI에서 그룹핑 |
| `@Operation` | 메서드 | 개별 API의 요약/설명/주석 작성 |
| `@Parameter` | 메서드 파라미터 | 개별 파라미터 설명 및 예시/기본값 지정 |
| `@RequestBody` | 메서드 파라미터 | 요청 본문 객체의 설명 및 스키마 자동생성 |
| `@ApiResponse` | 메서드 | 응답 코드별 설명 및 스키마 지정 |
| `@Schema` | DTO 클래스/필드 | 필드별 설명, 예시, 타입, 필수여부, 형식 등 |
| `@Hidden` | 클래스/메서드/필드 | 문서화에서 숨기기(표시되지 않음) |
---
### \[참고\] OAS 문서 작성 예시
...
```
실행 후 `http://localhost:8080/swagger-ui.html`에 접속하면 컨트롤러별로 그룹핑된 API 목록을 확인할 수 있으며, 각 엔드포인트의 파라미터·요청 바디·응답 스키마·응답 코드가 자동으로 문서화되어 표시됨. `http://localhost:8080/v3/api-docs`에서는 해당 내용이 OpenAPI 3.1.0 JSON 형식으로 제공됨.
---
### Actuator란?
...
| Metrics | JVM, CPU, 메모리, GC, 요청 수 등 성능 지표 제공 | `/actuator/metrics` |
| Prometheus Metrics | Metrics 정보를 Prometheus 표준으로 전환하여 제공 | `/actuator/prometheus` |
| Info | 앱 버전, 빌드 정보 등 메타데이터 제공 | `/actuator/info` |
| Env | 환경 변수, 설정 값 노출 | `/actuator/env` |
| Loggers | 로깅 레벨 동적 변경 가능 | `/actuator/loggers` |
...
기본 접속 주소는 `http://localhost:8080/actuator`이며, `management.server.port` 지정에 따라 포트가 변경될 수 있음. 기본값은 `server.port`와 동일함. 접속 시 활성화된 모든 엔드포인트 목록이 JSON `_links` 형태로 반환됨.
Spring Boot 애플리케이션의 상태 확인·시스템 모니터링·관리(Management)·운영 자동화 등을 위한 운영용 엔드포인트(REST API)를 자동으로 제공함.
| 기능 구분 | 설명 | 엔드포인트 |
| --- | --- | --- |
| 헬스 체크 | 시스템, DB 등 주요 컴포넌트 상태 확인 | `/actuator/health` |
| 애플리케이션 정보 | 버전, 이름 등 info 정보 | `/actuator/info` |
| 메트릭 | JVM, CPU, 메모리, 트래픽, GC, 요청 수, response time 등 | `/actuator/metrics` |
| 환경변수 | 환경, 프로퍼티, 시스템 변수 | `/actuator/env` |
| 빈 정보 | 스프링 빈 목록, 상태, 의존관계 | `/actuator/beans` |
| 스레드 덤프 | 스레드 상태/스택 추적 | `/actuator/threaddump` |
| 로그 레벨 동적 변경 | 런타임에 로그 레벨 변경 | `/actuator/loggers` |
| 캐시 정보 | Spring Cache 정보 | `/actuator/caches` |
| HTTP 트레이스 | 최근 HTTP 요청/응답 추적 | `/actuator/httptrace` |
| Mappings | RequestMapping 정보 | `/actuator/mappings` |
---
...
---
### Actuator - Health
기본 엔드포인트 `/actuator/health`에서 애플리케이션의 전반적인 상태를 표시함.
Health 상태 유형.
...
| Indicator | 역할 | 판단 기준 |
| --- | --- | --- |
| DiskSpaceHealthIndicator | 디스크 용량 확인 | 여유 공간이 기준치 미만이면 DOWN |
| DataSourceHealthIndicator | DB 연결 확인 | DB 연결 불가 시 DOWN |
| RedisHealthIndicator | Redis 연결 확인 | Redis 서버 미응답 → DOWN |
| MongoHealthIndicator | MongoDB 연결 확인 | 연결 실패 → DOWN |
| CassandraHealthIndicator | Cassandra 연결 확인 | 연결 실패 → DOWN |
| PingHealthIndicator | 단순 UP 체크 | 기본은 항상 UP |
사용자가 Indicator를 별도 구성 가능함 (`HealthIndicator`를 상속받아서 구현).
---
...
| 구분 | 엔드포인트 | 의미 | 상태 정보 |
| --- | --- | --- | --- |
| Readiness Probe | `/actuator/health/readiness` | 지금 트래픽을 받을 준비가 되었나? | ACCEPTING_TRAFFIC → 200 OK / REFUSING_TRAFFIC → 503 (시작 전/그레이스풀 셧다운 중/사용자 전환 등) |
| Liveness Probe | `/actuator/health/liveness` | 프로세스가 살아있어(진행 가능 상태야)? | CORRECT → 200 OK / BROKEN → 503 (앱 실패/컨텍스트 깨짐 등) |
---
...
![]()
Kubernetes의 `kubelet`이 주기적으로 두 엔드포인트를 탐침(probe)하여 다음과 같이 동작함.
