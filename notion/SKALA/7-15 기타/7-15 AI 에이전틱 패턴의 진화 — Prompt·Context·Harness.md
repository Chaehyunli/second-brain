---
title: "AI 에이전틱 패턴의 진화 — Prompt·Context·Harness"
notion_page_id: "39e1d84b-f68e-8196-a1d2-c7b8ebef0c56"
notion_url: "https://app.notion.com/p/39e1d84bf68e8196a1d2c7b8ebef0c56"
source_url: "https://bits-bytes-nn.github.io/insights/agentic-ai/2026/04/05/evolution-of-ai-agentic-patterns.html"
type: "manual-learning-note"
---

# [7/15] AI 에이전틱 패턴의 진화 — Prompt·Context·Harness

## 연결
- 상위 개념: [[7-15 Prompt 설계 및 Context Engineering]]
- Harness 구조: [[7-15 Agent Harness 핵심 구조 — LangChain]]
- 코딩 에이전트 사례: [[7-15 OpenCode로 보는 코딩 에이전트 내부 구조]]

## 원문
[프롬프트에서 하네스까지 — AI 에이전틱 패턴 4년의 기록](https://bits-bytes-nn.github.io/insights/agentic-ai/2026/04/05/evolution-of-ai-agentic-patterns.html)

## 핵심
- 흐름은 **Prompt Engineering → Context Engineering → Harness Engineering**으로 확장됐다.
- 좋은 지시문만으로는 한계가 있어, 어떤 정보·상태를 컨텍스트에 제공할지와 이를 운영하는 시스템 구조가 중요해졌다.
- 에이전트의 품질은 프롬프트 문장 하나보다 컨텍스트 재사용, 도구·검증 루프, 하네스 복잡도를 어떻게 설계하는지에 좌우된다.
