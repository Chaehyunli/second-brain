---
title: "[8/18]Spring AI_Day1_핵심 정리"
notion_page_id: "3bf1d84b-f68e-808c-a664-c3b11ea6d083"
source_url: "https://app.notion.com/p/3bf1d84bf68e808ca664c3b11ea6d083"
synced_at: "2026-08-19T00:10:23+09:00"
content_sha256: "41cd938eb3e05fa2a650227393f650214f01e113db7420fed66244ec9ca93055"
tags: [notion, skala, learning, spring-ai, spring-boot, llm, rag]
---

# [8/18]Spring AI_Day1_핵심 정리

[[notion/SKALA/index|SKALA 학습 노트]]

> 원문: [Notion 페이지](https://app.notion.com/p/3bf1d84bf68e808ca664c3b11ea6d083) (2026-08-19 KST 확인)
>
...
| 용어 | 한 줄 정의 | 알아 둘 점 |
| --- | --- | --- |
| 토큰(token) | 모델이 글을 자르는 단위 | 비용·길이의 기준. 한글은 글자당 1\~2토큰 |
| 컨텍스트 윈도우 | 한 요청에 담을 수 있는 총 토큰 | 넘으면 오래된 대화부터 잘려 나감 |
| 프롬프트 | 모델에 보내는 입력 전체 | 지시·맥락·예시·출력형식 네 요소 |
| 시스템 메시지 | 역할과 규칙을 정하는 첫 메시지 | 사용자 입력과 반드시 분리해 둠 |
| 온도(temperature) | 답의 무작위성 | 분류·추출은 0, 창작만 올림 |
| top-p | 확률 상위 몇 %까지 후보로 볼지 | 온도와 동시에 건드리지 않음 |
| 할루시네이션 | 모르는 것을 그럴듯하게 지어내는 현상 | 근거(RAG) + "모르면 모른다" 지시로 줄임 |
| 파인튜닝 | 모델 가중치를 추가로 학습시키는 것 | 지식 주입보다 말투·형식에 효과적 |
| 멀티모달 | 텍스트 외 이미지·음성도 입력 | 모델이 지원해야 쓸 수 있음 |
...
| 용어 | 한 줄 정의 | 알아 둘 점 |
| --- | --- | --- |
| 임베딩(embedding) | 문장을 숫자 벡터로 바꾼 것 | 뜻이 가까우면 벡터도 가까움 |
| 차원(dimension) | 벡터의 길이 (예: 1536) | 모델마다 다름 — 바꾸면 전체 재색인 |
| 코사인 유사도 | 두 벡터의 각도로 재는 닮은 정도 | 1에 가까울수록 비슷함 |
| 벡터 DB | 벡터·원문·메타데이터를 함께 저장·검색 | pgvector · Chroma · Qdrant 등 |
| 청킹(chunking) | 문서를 검색 단위로 자르는 일 | 800\~1200자, 겹침 10\~20%에서 시작 |
| 겹침(overlap) | 이웃 청크가 공유하는 앞뒤 문장 | 근거가 반토막 나는 것을 막는 보험 |
| top-k | 검색해서 가져올 청크 개수 | 크면 근거도 늘지만 잡음·비용도 늘어남 |
| MMR | 유사도와 다양성을 함께 보는 검색 | 비슷한 청크가 몰리는 것을 막음 |
| 인덱스(HNSW·IVF) | 벡터를 빠르게 찾기 위한 자료구조 | 건수가 커지면 반드시 필요해짐 |
...
| 용어 | 한 줄 정의 | 알아 둘 점 |
| --- | --- | --- |
| RAG | 검색한 근거를 붙여 답하게 하는 방식 | 재학습 없이 지식을 갱신함 |
| 인제스트(ingest) | 문서를 읽어 자르고 임베딩해 저장 | 보통 배치로 미리 해 둠 |
| 리트리버(retriever) | 질문으로 관련 청크를 찾아 주는 것 | Spring AI는 VectorStore·Advisor가 담당 |
| 근거·출처 | 답의 바탕이 된 문서 조각과 그 위치 | 출처 없는 RAG는 반쪽임 |
| 재순위(rerank) | 후보를 다시 정렬해 상위만 남기기 | 품질 대비 효과가 가장 확실함 |
| 하이브리드 검색 | 키워드(BM25) + 의미 검색 결합 | 제품 코드·고유명사에 강함 |
| 필터(filter) | 메타데이터 조건으로 검색 범위 제한 | 권한은 프롬프트가 아니라 필터로 |
| 컨텍스트 재현율/정밀도 | 근거가 검색됐나 / 상위에 왔나 | 나쁜 쪽을 먼저 고침 |
| QuestionAnswerAdvisor | RAG 조립을 대신해 주는 Spring AI 부품 | 검색 → 프롬프트 결합을 자동화함 |
...
| 용어 | 한 줄 정의 | 알아 둘 점 |
| --- | --- | --- |
| Tool Calling | 모델이 함수를 호출하도록 하는 기능 | 판단은 모델, 실행은 우리 코드 |
| 도구 스키마 | 모델이 보는 함수 이름·설명·파라미터 | 설명이 곧 스펙 — 잘 쓰면 잘 부름 |
| ReAct | 생각과 행동을 번갈아 하는 패턴 | 도구 결과를 보고 다시 판단함 |
| 에이전트 루프 | 도구 호출과 판단의 반복 | 반드시 상한을 둠 — 비용은 무한함 |
| MCP | 도구를 붙이는 표준 규약 | 우리 도구를 남에게 열 수도 있음 |
| 승인 게이트·HITL | 되돌리기 어려운 행동 전 사람 확인 | 환불·발송·삭제 |
| 가드레일 | 입력·출력에 거는 안전 규칙 | 차단은 메모리 저장보다 앞에 |
| 프롬프트 인젝션 | 지시를 무시하게 만드는 공격 | 문서에 심어 오는 간접형도 있음 |
| 멀티 에이전트 | 역할을 나눈 에이전트들의 협업 | 나누기 전에 도구부터 정리함 |
...
| 용어 | 한 줄 정의 | 알아 둘 점 |
| --- | --- | --- |
| ChatClient | 모델 호출의 표준 진입점 | 빌더로 기본값을 미리 넣어 둠 |
| Advisor | 요청·응답에 공통 처리를 끼우는 장치 | 순서가 곧 정책 — 차단은 저장보다 앞 |
| VectorStore | 벡터 저장·검색 추상화 | 인메모리 → pgvector 교체가 설정 한 줄 |
| DocumentReader·Splitter | 문서 읽기·분할 부품 | 인제스트의 앞 두 단계 |
| ChatMemory | 대화 이력 저장·주입 | 대화 ID 규칙을 한 곳에 모음 |
| 구조화 출력 | 응답을 객체로 받는 것 | `.entity(Class)` — 문자열 파싱하지 않음 |
| 스타터·BOM | 의존성 묶음과 버전 정렬 | 버전은 BOM 한 줄로 맞춤 |
| ToolContext | 도구에 안전하게 넘기는 값 묶음 | 사용자 ID는 프롬프트가 아니라 여기로 |
| 프로파일 | 환경별 설정 분리 | 개발·운영 설정을 나눔 |
Advisor는 Spring의 인터셉터/필터 개념과 유사함. 체인 순서가 정책을 결정하므로, 가드레일·로깅 Advisor는 메모리·RAG Advisor보다 앞에 두어야 함. `ToolContext`는 도구 호출 시 모델이 아닌 서버 컨텍스트(인증 정보, 사용자 ID 등)를 안전하게 전달하는 전용 채널임.
---
### 계층 구조 — 쉽게 말하면
...
| --- | --- | --- |
| 주문 접수 창구 | Controller — 요청을 받는다 | 창구 직원이 창고까지 뛴다 |
...
| 갈래 | 어노테이션 | 무엇을 하나 |
| --- | --- | --- |
| 빈 등록 | `@Component` · `@Service` · `@Repository` · `@Controller` | 클래스를 스프링 부트가 관리하는 객체(빈)로 등록함 |
| 요청 매핑 | `@RestController` · `@GetMapping` · `@PostMapping` | HTTP 요청을 메서드에 연결함 |
| 주입 | 생성자 주입(권장) · `@Autowired` · `@Qualifier` | 필요한 빈을 찾아 넣어 줌 |
| 설정 | `@Configuration` · `@Bean` · `@ConfigurationProperties` | 내가 직접 만드는 빈과 외부 설정 바인딩 |
| 검증·예외 | `@Valid` · `@RestControllerAdvice` · `@ExceptionHandler` | 입력을 검증하고 예외를 응답으로 바꿈 |
| 부가 기능 | `@Transactional` · `@Async` · `@Retryable` · `@Aspect` | 본래 코드를 건드리지 않고 행동을 덧붙임 |
이름이 다른 이유는 의도를 드러내기 위해서이고, 기능은 거의 같음.
---
### `@SpringBootApplication`의 정체
`@SpringBootApplication`은 세 어노테이션을 합친 것임.
```java
...
```
Spring AI의 자동 구성도 `@EnableAutoConfiguration`에 얹혀 동작함.
---
### 스테레오타입 — 이름이 다른 이유
`@Service` · `@Repository` · `@Controller`는 모두 `@Component`의 특수형임. 빈 등록 기능은 같지만 읽는 사람에게 역할을 알려 줌.
```java
...
```
`@Repository`만 예외 변환이라는 실제 추가 기능이 있음.
---
### 빈과 의존성 주입 — 쉽게 말하면
...
받고·검증하고·서비스에 넘기고·응답 형태로 바꿔 돌려주는 역할만 함. 업무 규칙을 넣지 않으며, `if`문이 늘어나기 시작하면 서비스로 옮길 신호임. 요청·응답은 DTO(record)로 받고, 엔티티를 그대로 노출하지 않음.
```java
@RestController
...
```
컨트롤러에 `if`가 쌓이기 시작하면 업무 규칙이 새어 들어온 것임. 검증은 `@Valid`에, 판단은 서비스에 맡김.
---
### `@Service` — 업무 흐름과 트랜잭션
여러 Repository·외부 호출을 조합해 하나의 업무를 완성함. `@Transactional`로 경계를 긋고, 조회 전용 메서드는 `readOnly = true`로 선언함.
```java
@Service
...
| --- | --- | --- |
| SQL | 메서드 이름·JPQL로 생성됨 | 내가 직접 씀 |
| 잘 맞는 일 | 단건 CRUD · 엔티티 상태 변경 | 동적 검색 · 집계 · 리포트 |
| 돌려주는 것 | 엔티티 — 영속 상태·변경 감지 | 조회 전용 DTO — 그냥 값 |
| 쓰기 | `save()` 하나로 update까지 | update 문을 직접 씀 |
| 튜닝 | 생성된 SQL을 확인해야 앎 | 보이는 그대로 고침 |
| 같이 쓸 때 | 쓰기·단건 조회를 맡음 | 목록·통계 화면을 맡음 |
---
### DTO — 쉽게 말하면
...
---
### DTO와 엔티티 — 계층의 경계
엔티티를 API로 그대로 내보내면 DB 구조가 곧 API 스펙이 됨. 컬럼 하나 바꿨을 뿐인데 클라이언트가 깨짐. `record`로 요청·응답 DTO를 만들고 변환은 한 곳에서 함.
```java
...
| 방식 | 언제 쓰나 | 대가 |
| --- | --- | --- |
| DTO 안 정적 팩토리 | 변환이 한두 개·다른 빈이 필요 없을 때 | DTO가 엔티티를 알게 됨 |
| 매퍼 컴포넌트(`@Component`) | 변환에 규칙·다른 빈이 필요할 때 | 클래스가 하나 늘음 |
| MapStruct | 필드가 많고 같은 변환이 반복될 때 | 빌드 설정·생성 코드를 읽을 줄 알아야 함 |
| SQL이 직접 채움 | 조회 전용 화면 — 가장 빠름 | 같은 모양을 여러 곳에서 쓰기 어려움 |
```java
...
---
### 의존성 주입 — 생성자 주입이 기본
필드 주입(`@Autowired`)은 테스트가 어렵고 순환 참조를 숨김. 생성자 주입은 `final`을 쓸 수 있어 불변이고, 누락 시 컴파일 오류가 발생함. 생성자가 하나면 `@Autowired`도 생략할 수 있음.
```java
...
내 코드가 아닌 클래스(라이브러리)는 `@Component`를 붙일 수 없음. 그럴 때 `@Configuration` 클래스에서 `@Bean`으로 직접 만듦. Spring AI의 `ChatClient` · `VectorStore`도 이 방식으로 구성함.
```java
@Configuration
...
```
`@ConfigurationProperties`를 쓰면 `application.yml`의 값이 타입 안전하게 바인딩되어 코드에 하드코딩된 상수를 남기지 않아도 됨.
---
### AOP — 본래 코드를 건드리지 않고
로깅·감사·측정처럼 여기저기 흩어지는 코드를 한곳에 모으는 방법임. `@Transactional`도 사실 AOP고, Spring AI의 Advisor(12장)도 발상이 같음.
```java
@Aspect
@Component
...
```
AOP는 프록시 기반이므로 같은 클래스 내부에서 `this.xxx()`로 호출하면 Aspect가 적용되지 않음. 이 제약은 `@Transactional`에도 동일하게 적용됨.
---
### 입력 검증과 예외 처리
검증은 DTO에 선언하고 `@Valid`로 켬 — 컨트롤러에 `if`를 쌓지 않음. 예외는 던지고, 응답으로 바꾸는 일은 `@RestControllerAdvice` 한 곳에서 함.
```java
...
| --- | --- | --- |
| 검증 — Controller | item을 비우고 주문 생성 | 400 + 항목별 메시지(서비스는 실행조차 안 됨) |
| 권한 — Service·SQL | user1로 99999 조회 | 404 — 남의 주문은 '없는 것'으로 |
| 동적 조건 — Mapper | status·keyword를 넣었다 뺐다 | 조건 수에 따라 결과가 바뀜 |
| 집계 — Mapper | /statistics 호출 | SQL 한 번으로 계산된 건수·합계 |
| 인증 — Security | user1로 관리자 API 호출 | 403 — 문서에 적힌 그대로 |
| 응답 시간 | 요청마다 표시되는 duration | AI 호출은 이 숫자가 곧 사용자 경험 |
---
### 프로파일 — 환경별로 다르게
개발·운영의 차이는 코드가 아니라 설정으로 표현함. `application-{profile}.yml`이 공통 설정을 덮어씀. `@Profile`로 빈 자체를 갈아 끼울 수도 있음.
```yaml
# application.yml — 공통
helpdesk:
...
---
### AI는 어느 계층에 두나
`ChatClient`를 컨트롤러에서 직접 부르는 것이 가장 흔한 실수임. AI는 새로운 계층이 아니라 기존 계층에 얹히는 하나의 관심사 — 그래서 자리를 정해 줘야 함.
...
**ChatClient의 정의:** Spring AI에서 제공하는 LLM 호출 전용 인터페이스(Fluent API)입니다. Spring의 `RestClient`나 `WebClient`처럼 `chatClient.prompt().user(...).call().content()` 형태로 프롬프트 작성과 모델 호출을 담당
**Advisor(어드바이저)란?**
- **개념:** 스프링의 인터셉터(Interceptor)나 **AOP**와 같은 개념. LLM에 요청을 보내기 **전**과 응답을 받은 **후**를 낚아채서 공통 관심사를 처리해 주는 모듈
- **주요 역할:**
...
| 항목 | 한 줄 정리 | 실무 포인트 |
| --- | --- | --- |
| 계층 분리 | 변경의 파급을 가두기 위해 나눔 | 위→아래로만 호출, 역방향 금지 |
| `@RestController` | 받고·검증하고·돌려줌 | if가 쌓이면 서비스로 옮길 신호 |
| `@Service` | 업무 흐름과 트랜잭션 경계 | 클래스 readOnly, 쓰기만 재정의 |
| `@Repository` | 데이터에 닿는 유일한 곳 | 권한 조건은 쿼리 안에 |
| Mapper(MyBatis) | SQL이 주인공인 조회를 맡음 | Repository와 같은 자리, 다른 방식 |
| DTO | 계층 사이의 방화벽 | 엔티티를 API로 내보내지 않음 |
| 생성자 주입 | 의존성이 시그니처에 드러남 | 5개 넘으면 책임 과다 신호 |
| `@Bean` | 라이브러리 타입을 빈으로 | 파라미터도 주입 — AI 설정의 기본형 |
| AI 배치 | Config·Advisor·Service로 분산 | 컨트롤러는 AI를 모름 |
| 변환 | 엔티티 ↔ DTO는 한 곳에서 | 입구가 둘이어도 출구는 하나 |
| API 문서 | 코드에서 생성 — Swagger UI로 시험 | 운영 프로파일에서는 닫음 |
---
### LLM과 토큰 — 쉽게 말하면
...
| 개념 | 무엇인가 | 왜 알아야 하나 |
| --- | --- | --- |
| 토큰 | 모델이 다루는 텍스트 조각(한글 ≈ 1\~2자) | 비용·속도·상한이 모두 토큰 단위 |
...
| 원칙 | 의미 |
| --- | --- |
| Portability | 공급자가 달라도 같은 인터페이스로 씀 |
...
- **역할:** `EmbeddingModel`이 변환한 벡터 데이터를 저장하고, 사용자의 질문과 가장 유사한 의미를 가진 근거 문서를 찾아내는 DB 추상화 인터페이스입니다.
...
---
### ① ChatModel — 대화의 기본
가장 기본이 되는 추상화 — 프롬프트 → 텍스트 응답. 공급자별 구현(OpenAI·Anthropic·Azure OpenAI 등)을 같은 인터페이스로 씀. 저수준 API이므로 실무에선 대개 `ChatClient`로 감싸서 씀.
```java
@Service
...
| 좌표를 모아 둔 지도책 | 벡터 DB(VectorStore) | 질문과 가까운 문단을 찾아 줌 |
...
---
### 공급자별 옵션 — 공통과 고유
공통 옵션(`model` · `temperature` · `maxTokens`)은 `ChatOptions`로 동일하게 씀. 공급자 고유 옵션은 각자의 `XxxChatOptions`로만 지정함. 고유 옵션을 쓰는 순간 그 코드는 그 공급자에 묶임 — 경계를 알고 써야 함.
...
| 단계 | 무엇을 한다 | 확인 기준 |
| --- | --- | --- |
| ① | SpringAI_실습/00_참조예제 실행 | 기동 로그에 포트 8080 |
...
| ChatModel | 프롬프트 → 텍스트, 가장 낮은 계층 | 실무에선 ChatClient로 감싸 씀 |
...
| AutoConfiguration | 스타터 + yml이면 빈이 자동 구성 | 코드가 아니라 설정으로 바꾼다 |
---
### 의존성 설정 (build.gradle)
```plain text
plugins {
...
| 단계 | 권장 | 주의 |
| --- | --- | --- |
| 개인 개발 | VS Code launch.json · 셸 환경변수 | 셸 히스토리에 남을 수 있음 |
| 팀 공유 | 공유하지 않음 — 각자 발급 | 한 키 돌려쓰면 사고 추적 불가 |
| CI | 저장소 Secret | PR 빌드에는 노출 주의 |
| 운영 | K8s Secret · Vault | 환경변수 덤프 엔드포인트 차단 |
```bash
# ❌ 절대 하지 않는다
...
| 증상 | 원인 | 해결 |
| --- | --- | --- |
| 기동은 되는데 호출하면 502 | 환경변수 미설정 | `export OPENAI_API_KEY=...` 후 재시작 |
...
| 항목 | 한 줄 정리 | 실무 포인트 |
| --- | --- | --- |
| JDK | 21 · toolchain으로 고정 | "제 컴퓨터에선 되는데"를 막음 |
| VS Code | 확장 팩 · UTF-8 · launch.json | 한글 깨짐 · 키 커밋을 막음 |
| 프로젝트 | 의존성 4개면 시작 가능 | 모델 스타터는 하나만 |
| API 키 | 발급 → 한도 먼저 → 보관 | 커밋했으면 폐기가 우선 |
| 벡터 DB | 8장 전까지 인메모리로 충분 | 재시작마다 재적재면 교체 신호 |
| 첫 실행 | /hello 응답 한 줄 | 여기 막히면 진도 나가지 않음 |
---
### BOM과 스타터
...
| spring-ai-bom | 버전 일괄 관리 | 여기 하나만 버전을 적음 |
---
### application.yml — 기본 설정
공급자 설정은 `application.yml`에 선언 — 코드가 아님. API 키는 환경변수로 주입 — 소스에 넣지 않음.
```yaml
spring:
  ai:
    openai:
...
| 3 | `application-{profile}.yml` | 환경별 차이만 담음 |
...
| 이렇게 생각하면 쉽다 | 실제로는 | 왜 그렇게 하나 |
| --- | --- | --- |
| 햄버거 주문서 | 빌더(Builder) | 필요한 옵션만 골라 담음 |
| "치즈 추가, 양파 빼고" | 메서드 체이닝 | 한 문장으로 이어서 말함 |
| 점원이 주문서를 다시 건네준다 | 메서드가 자기 자신을 돌려줌 | 그래서 점을 또 찍을 수 있음 |
| 마지막에 "주문할게요" | `build()` · `call()` | 이때 비로소 만들어지고 불림 |
| 주문서만 쓰고 안 내면 | 종료 메서드를 빠뜨림 | 아무 일도 안 남 — 흔한 실수 |
```java
...
| 이렇게 생각하면 쉽다 | 실제로는 | 이득 |
| --- | --- | --- |
| 부서별 전화 창구 | 용도별 ChatClient 빈 | 요약용·분류용을 따로 둠 |
| 창구마다 정해진 응대 지침 | 기본 시스템 프롬프트 | 매번 지시를 다시 안 써도 됨 |
| 말 빠르기·길이 규정 | 온도·최대 토큰 | 답이 매번 달라지지 않음 |
| 창구를 바꿔도 절차는 같다 | 모델 교체는 설정 한 줄 | 코드를 안 고침 |
---
### ChatClient 빈 — 용도별로 나눈다
하나의 `ChatClient`로 모든 일을 시키면 기본값이 서로 충돌함. 요약용·분류용·상담용처럼 용도별 빈을 만들어 이름으로 주입함.
```java
@Configuration
class AiConfig {
...
| 방식 | 반환 | 적합 | 주의 |
| --- | --- | --- | --- |
| `.call().content()` | String | 짧은 답·분류·추출 | 긴 답변은 체감이 나쁨 |
| `.call().entity(T)` | 객체 | 구조화 응답 API | 형식 실패 대비 필요 |
| `.call().chatClientResponse()` | 응답+컨텍스트 | 출처·메타데이터 필요 | 가장 정보가 많음 |
...
---
### 응답 메타데이터 — 무엇이 함께 오나
응답에는 텍스트 말고도 운영에 필요한 정보가 함께 옴. `finishReason`이 `length`면 답이 잘린 것임 — 그냥 넘기면 잘린 JSON 파싱이 실패하거나 문장이 끊긴 답이 사용자에게 나감.
```java
...
| --- | --- | --- |
| 누구에게 시키는가 | 시스템 메시지 — 역할·규칙 | 말투와 기준이 매번 바뀜 |
| 무엇을 해 달라 | 지시 — 동사로 분명하게 | 엉뚱한 것을 해 옴 |
| 참고 자료 | 맥락 — 근거 문서·데이터 | 아는 대로 지어냄 |
| 견본 두어 개 | 예시(Few-shot) | 경계 사례에서 흔들림 |
| 제출 양식 | 출력 형식 지정 | 매번 모양이 달라 파싱이 깨짐 |
...
| 개념 | 한 줄 정리 | 실무 포인트 |
| --- | --- | --- |
| BOM | 모듈 버전을 한 번에 맞춤 | 버전 올릴 때 이 한 줄만 바꿈 |
| application.yml | 공급자·모델·옵션을 선언 | API 키는 환경변수로만 |
| ChatClient | ChatModel 위의 Fluent API | Advisor·객체 변환까지 얹혀 있음 |
| 빌더 기본값 | system·options·advisors를 미리 | 용도별로 빈을 나눔 |
| 파라미터 바인딩 | `{변수}` + `.param()` | 문자열 연결은 인젝션 표면을 넓힘 |
