---
title: "[8/12] Java, SpringBoot, Rest API 구현_Day3_핵심 정리"
notion_page_id: "3b91d84b-f68e-801f-a416-e4f1b29fd093"
source_url: "https://app.notion.com/p/3b91d84bf68e801fa416e4f1b29fd093"
synced_at: "2026-08-13T08:02:06+09:00"
content_sha256: "3c70e607bfd45aa802bfd27bac37f016d030321d1ed84b0c58263f39f0803457"
tags: [notion, skala, learning, java, spring-boot, rest-api]
---

# [8/12] Java, SpringBoot, Rest API 구현_Day3_핵심 정리

[[notion/SKALA/index|SKALA 학습 노트]]

> 원문: [Notion 페이지](https://app.notion.com/p/3b91d84bf68e801fa416e4f1b29fd093) (2026-08-13 확인)

### 객체 생성 패턴 (Creational Patterns) 개요
객체의 생성 과정을 추상화하여, 시스템이 구체적인 클래스에 의존하지 않고 객체를 만들 수 있게 해주는 패턴군임.
| 패턴명 | 주요 목적/역할 | 대표 예시 |
| --- | --- | --- |
| Singleton | 단 하나의 인스턴스만 생성·공유 (전역에서 하나의 객체만 필요할 때) | `Runtime`, `Logger` |
| Factory Method | 객체 생성을 하위 클래스에서 담당. 상위 클래스는 인터페이스만 제공 | `Calendar.getInstance()` |
| Abstract Factory | 관련 객체 집합을 통일된 방식으로 생성 (객체 계열 묶음 생성) | GUI Toolkit, DAO Factory |
| Builder | 복잡한 객체의 생성 절차를 단계별로 분리, 체이닝 지원 | `StringBuilder`, Lombok `@Builder` |
---
### Singleton
시스템 전체에서 **단 하나의 인스턴스**만을 제공하며, 어디서든 동일한 객체에 접근할 수 있도록 보장하는 패턴임.
...
```
스프링에서는 `@Component`, `@Service`, `@Repository`로 클래스를 선언하면 Spring IoC 컨테이너가 자동으로 싱글톤 객체로 관리함 → 직접 구현 없이 싱글톤 보장됨.
---
### Factory Method (1/2) — 구조
객체 생성 코드를 별도의 `factoryMethod`에 위임하여, **객체 생성 로직과 처리 로직을 분리**하는 패턴임. 상위 클래스(Creator)는 어떤 객체를 만들지 모르고, 하위 클래스(ConcreteCreator)가 결정함.
- 장점: 코드의 결합도 감소, 확장성 및 유지보수성 증가
- 단점: 클래스 수 증가, 코드 구조가 복잡해질 수 있음
UML 구조: `Creator`(abstract) → `ConcreteCreator`가 상속, `Product`(abstract) → `ConcreteProduct`가 상속. `ConcreteCreator`의 `factoryMethod()`가 `ConcreteProduct`를 생성해 반환함.
```java
...
처리 로직(`ProductProcessor`)은 구체적인 구현체를 전혀 알지 못하고, **`Creator`**** 인터페이스만 의존**함. 실제로 어떤 객체가 생성되는지는 외부에서 주입받은 `Creator`가 결정함 → "How는 포함되어 있지 않음".
```java
// 처리 로직 — 생성과 완전히 분리
...
```
`ProductProcessor`는 `ConcreteProduct`를 직접 참조하지 않으므로, 나중에 다른 `ConcreteCreator`로 교체해도 처리 로직을 수정할 필요가 없음.
---
### Abstract Factory — 구조
관련성 있는 여러 종류의 객체군(패밀리)을 생성하는 인터페이스를 제공하는 패턴임. Factory Method가 단일 객체 생성에 집중한다면, Abstract Factory는 **연관된 객체들의 집합 전체**를 묶어서 생성함.
- 장점: 관련된 객체들을 일관되게 생성, 객체군(팩토리) 교체가 쉬움
- 단점: 새로운 제품(Product 종류) 추가가 어려움, 구조가 복잡해질 수 있음<br>→ `AbstarctFactory` 클래스에 새로운 메소드로 `CreateProductC` 가 추가될 경우 하위의 `ConcreteFactory` 클래스에도 메소드를 추가해야함
UML 구조:
...
- `Client`는 `AbstractFactory`만 바라보고, 어떤 ConcreteFactory인지 신경 쓰지 않음
---
...
app.renderUI();
```
`Application`은 `GUIFactory` 인터페이스만 알고, 내부에서 `createButton()`/`createCheckbox()`를 호출함 → 팩토리만 교체하면 전체 UI 계열이 바뀜.
---
### Builder
복잡한 객체를 **단계별로 생성**할 수 있게 도와주는 패턴임. 각 단계는 메서드 체이닝 방식으로 구현됨.
- 장점: 객체 생성 과정 분리, 불변 객체 생성 가능, 가독성 향상
- 단점: 클래스 구조가 복잡해질 수 있음
- 참고: 중첩 클래스 간에는 `private` 속성에 서로 접근 가능함 → `Builder`가 `User`의 `private` 생성자를 호출할 수 있는 이유임
```java
class User {
    private String name;
    private int age;
...
---
### SOLID 원칙이란?
유지보수성, 확장성, 재사용성을 높이고 결합도(coupling)를 낮추기 위한 **OOP의 5대 설계 원칙**임. Clean Architecture(지속 가능하고 튼튼한 소프트웨어 구조)를 구성하기 위한 핵심 기초 원리이기도 함.
SOLID를 큰 그림에서 보면:
...
| O | 개방-폐쇄 원칙 (OCP) | 기존 구성을 수정하지 않고 확장할 수 있어야 하며, 기존 구성은 변경으로부터 폐쇄되어야 함 |
| L | 리스코프 치환 원칙 (LSP) | 자식 클래스로 교체해도 시스템 전체 행위가 일관되게 유지되어야 함 |
| I | 인터페이스 분리 원칙 (ISP) | 모듈/서비스는 자신이 사용하지 않는 불필요한 기능에 강제로 묶이지 않아야 함 |
| D | 의존 역전 원칙 (DIP) | 고수준의 비즈니스 로직은 저수준의 구체적 가변 요소에 의존해서는 안 됨 |
SOLID는 객체지향에서 시작되었지만, 객체 뿐만 아니라 마이크로서비스와 API Level에까지 적용을 고려함.
---
### 1. SRP — 단일 책임 원칙
한 클래스가 변경되어야 할 이유는 **오직 하나**여야 함. 한 클래스가 여러 기능을 담당하면, 한 기능의 변경이 다른 기능에 부정적 영향을 줄 수 있음.
```java
...
```
`NaverPay`를 추가하고 싶다면 `Payment`를 구현한 클래스만 새로 만들면 됨. `PaymentService`는 전혀 손대지 않아도 됨.
---
...
```
`Rectangle` 타입 변수에 `Square`를 넣고 `setWidth(5)` → `setHeight(3)`을 순서대로 호출하면, 직사각형이라면 넓이가 15여야 하지만 `Square`는 9가 됨. 부모의 계약(가로/세로 독립 변경)을 자식이 깨뜨린 것임.
---
### 3. LSP — 리스코프 치환 원칙 (2/2)
LSP를 지키는 올바른 방법은 **억지로 상속하지 않고, 공통 인터페이스를 각자 독립적으로 구현**하는 것임.
```java
// 1. 공통 규약을 인터페이스로 정의
interface Shape {
    int getArea();
...
```
두 클래스 모두 `Shape`로 업캐스팅해서 사용 가능하며, 서로의 계약을 침범하지 않음.
---
...
---
### 5. DIP — 의존 역전 원칙 (개요)
고수준 모듈은 저수준 모듈에 의존하면 안 되며, **둘 다 추상화에 의존**해야 함. 추상화는 구체적인 것에 의존해서는 안 되고, 구체적인 것이 추상화에 의존해야 함.
...
좋은 구조는 `Computer`가 `input device`, `display`라는 **추상화된 인터페이스**만 바라보고, 저수준 모듈들이 그 인터페이스를 구현하는 형태임. 새 장치를 추가해도 `Computer`는 수정 불필요함.
---
...
- `Computer`가 `Keyboard`와 `Monitor`의 구체 구현에 직접 의존
- 새로운 입력 장치(Mouse)가 추가되면 `Computer` 클래스를 수정해야 함
- `Keyboard`를 `WirelessKeyboard`로 교체하려 해도 `Computer`를 직접 수정해야 하며, OCP도 동시에 위반함
---
...
| OSI 7계층 | TCP/IP 4계층 | 주요 프로토콜 | HTTP 관점에서의 역할 |
| --- | --- | --- | --- |
| 응용·표현·세션 계층 (L5\~L7) | 응용 계층 (process 영역) | HTTP, FTP, DNS, DHCP | HTTP 요청/응답, URL, 메서드 / 포맷 변환, 암호화(TLS) / 세션 유지(쿠키, 세션ID, JWT) |
| 전송 계층 (L4) | 전송 계층 (OS Kernel) | TCP, UDP | port handling, socket |
| 네트워크 계층 (L3) | 인터넷 계층 (OS Kernel) | IP, ICMP | IP Handling |
| 데이터 링크·물리 계층 (L1\~L2) | 네트워크 액세스 계층 (NIC) | Ethernet, ATM | NIC에서 바이트로 해석 및 Frame 단위 묶음 / 공유기와 PC 간 무선·광신호 |
TCP/IP 모델에서 L3\~L4는 **OS Kernel**이 담당하고, L1\~L2는 **NIC(Network Interface Card)** 하드웨어가 담당함.
---
### \[참고\] OSI 레이어별 역할과 장비
OSI 계층 별로 서버 간 연결 방식과 담당 장비가 다름.
| 계층 | 주요 역할 | 주요 장비 | 설명 |
| --- | --- | --- | --- |
| L1 | 신호 전송 | 허브, 리피터 | 물리적 신호를 단순 중계, 충돌 많음 (구시대 장비) |
| L2 | 같은 네트워크 내 MAC 기반 통신 | L2 스위치 | 같은 네트워크 내 서버 MAC 주소 기반으로 패킷 전달 |
| L3 | 서로 다른 네트워크 간 통신 | 라우터 (L3 스위치) | IP 기반 라우팅, 외부 네트워크로 트래픽 전달 |
| L4 | TCP/UDP 포트 기반 트래픽 처리 | L4 로드 밸런서 | 포트/세션 기반 분산 처리 및 방화벽 기능 가능 |
| L5\~L7 | 세션 유지, 컨텐츠 기반 분산 | L7 로드 밸런서 (G/W) | HTTP 헤더, URL 등으로 로드 밸런싱, 보안 제어 |
...
전송 계층(Transport Layer)은 **패킷 흐름 제어와 오류 제어**를 담당하며, **포트 번호**를 통해 같은 호스트 내의 어느 애플리케이션으로 패킷을 전달할지 결정함.
하나의 호스트가 여러 애플리케이션을 동시에 실행하더라도, 전송 계층이 포트 번호를 보고 올바른 프로세스로 패킷을 배분함.
| 애플리케이션 | 포트 번호 |
| --- | --- |
| 웹 브라우저 ↔ 웹 서버 (HTTPS) | 443 |
| 메일 애플리케이션 ↔ 메일 서버 | 25 |
| 파일 전송 애플리케이션 ↔ 파일 서버 | 20 |
...
| 구분 | 인터넷 | 웹(WWW) |
| --- | --- | --- |
| 정의 | 네트워크 인프라 (하드웨어, 라우터, IP 등) | 인터넷 상의 문서·이미지·비디오 등 하이퍼링크로 연결된 정보들의 집합 |
| 역할 | 네트워크 연결과 데이터 통신을 담당 | 웹 브라우저로 접근 가능한 전 세계 정보 공간 접근 |
| 주요 프로토콜 | (TCP, UDP)/IP 등 | HTTP, HTTPS |
---
### 응용 계층 프로토콜: HTTP(S)
...
| HTTPS | `https://example.com` | 443 | 보안 웹 프로토콜 (TLS/SSL) |
| FTP | `ftp://user:pass@example.com:21/path/file` | 21 | 파일 전송 프로토콜 (업로드/다운로드) |
| FILE | `file:///C:/Users/example/file.txt` | 없음 | 로컬 파일 시스템 접근 |
| WS / WSS | `ws://example.com/socket`, `wss://...` | 80 / 443 | 웹소켓 (실시간 통신) |
---
### Request / Response 예시
#### GET Request Message
```plain text
...
소켓은 **OS 커널이 제공하는, 프로세스가 네트워크(IP/Port)를 통해 통신하기 위한 추상화된 통신 엔드포인트**임. OS Kernel 객체 + Socket API(유저 공간)로 구성됨.
- 커널이 연결 유지, Socket file Descriptor 관리, 포트 바인딩을 담당하고, 응용 프로그램이 socket API를 통해 통신함
- 소켓은 **특정 포트 번호와 연결**되어 있어서, TCP에서 데이터를 보낼 응용 프로그램을 식별하는 역할을 함
동작 예시: 웹 브라우저(소켓 포트 4289) → 인터넷 → 웹 서버(소켓 포트 80)
---
### 클라이언트와 서버 연결 흐름
소켓 기반 클라이언트-서버 연결은 3단계로 이루어짐.
1. 서버는 서버 소켓으로 들어오는 연결 요청을 기다림 (`listen` 상태)
2. 클라이언트가 서버에게 연결 요청을 보냄 (클라이언트 포트 → 서버 포트로 연결 요청)
3. 서버가 연결 요청을 수락하고 **새로운 소켓을 만들어** 클라이언트와 연결을 생성함 → 기존 `listen` 소켓은 계속 새 연결을 기다리고, 개별 클라이언트와의 통신은 새로 만들어진 소켓이 담당함
---
### InetAddress Class
`java.net.InetAddress`는 **IP 주소 + 호스트 이름(도메인)을 표현하는 클래스**임. DNS 조회, 로컬 호스트 정보 조회 등에 사용됨.
주요 역할:
- 도메인 이름 → IP 주소 변환
- IP 주소 → 도메인 이름 역방향 조회
- 로컬 머신의 IP/호스트명 조회
...
```
참고: `InetAddress`를 통해 Domain Name으로 IP를 찾을 때, 실제로는 OS Kernel의 glib resolver에게 IP를 찾도록 요청함 (`/etc/resolv.conf` 참조).
---
### Socket Class
`java.net.Socket`은 **TCP 클라이언트를 표현하는 클래스**임. 특정 서버(IP, Port)에 접속해서 데이터를 송수신함.
주요 역할:
...
| 메서드 | 설명 |
| --- | --- |
| `new Socket(String host, int port)` | 지정한 서버(host:port)에 TCP 연결 시도 |
| `getInputStream()` | 서버에서 오는 데이터 읽기 |
| `getOutputStream()` | 서버로 데이터 쓰기 |
| `close()` | 소켓 종료 |
---
### ServerSocket Class
`java.net.ServerSocket`은 **TCP 서버를 표현하는 클래스**임. 특정 포트에서 클라이언트 연결을 기다리고(리스닝), 연결되면 `Socket`을 하나 만들어 반환함.
주요 역할:
- 서버 포트 바인딩 (`new ServerSocket(port)`)
- `accept()` 호출로 클라이언트 연결 대기 (블로킹)
- 연결이 들어오면 `Socket` 인스턴스를 반환 → 이 `Socket`으로 실제 데이터 송·수신
주요 메서드:
| 메서드 | 설명 |
| --- | --- |
| `new ServerSocket(int port)` | 지정 포트에 서버 소켓 생성 및 바인딩 |
| `accept()` | 클라이언트 접속을 기다렸다가, 연결되면 `Socket` 반환 |
| `close()` | 서버 소켓 종료 (더 이상 새로운 연결 받지 않음) |
`ServerSocket`은 연결 수락 전담이고, 실제 데이터 통신은 `accept()`가 반환한 `Socket`이 담당함. 이 구조 덕분에 서버는 여러 클라이언트와 동시에 각각 독립된 소켓으로 통신할 수 있음.
---
### 웹 애플리케이션 개요
...
| --- | --- | --- |
| 주요 기능 | 정적 컨텐츠 제공 | 동적 컨텐츠 제공 |
| 역할 | HTML, CSS, JS, 이미지 등 정적 파일 반환 | 비즈니스 로직 처리, 동적 페이지 생성 |
| 예시 | Apache, Nginx | Tomcat, JBoss, WebLogic |
| 처리 방식 | 클라이언트 요청 → 파일 반환 | 클라이언트 요청 → 로직 처리 → 결과 반환 |
| DB 연동 | 불가 | 가능 |
| 속도 | 빠름 | 상대적으로 느림 |
| 실행 환경 | OS 레벨 | JVM 등 어플리케이션 레벨 |
...
| 요소 | 의미 | 질문 |
| --- | --- | --- |
| URI (리소스 주소) | 무엇을 제어할 것인가 | "어떤 자원이냐?" |
| HTTP Method (행위) | 어떻게 제어할 것인가 | GET, POST, PUT, DELETE 등 |
| Representation (표현) | 어떤 형태로 주고받을 것인가 | JSON, XML 등 |
---
...
| --- | --- | --- |
| 정의 | 리소스를 식별하는 모든 방법의 총칭 | 리소스의 위치(주소)를 명시하는 식별자 |
| 포함 관계 | URL은 URI의 하위 개념 | URI의 한 종류 |
| 형식 | `scheme:[//authority]path[?query][#fragment]` | `scheme://host[:port]/path[?query][#fragment]` |
| 예시 | `mailto:someone@example.com`, `https://example.com/index.html` | `https://example.com/index.html` |
| 기능 | 리소스의 식별 (이름, 위치 모두 가능) | 리소스의 위치 지정 및 접근 방법 명시 |
...
| --- | --- | --- | --- |
| 생성 (Create) | POST | /users | 새로운 사용자를 생성함 |
| 조회 (Read) | GET | /users/1 | 1번 사용자 정보를 가져옴 |
| 수정 (Update) | PUT 또는 PATCH | /users/1 | 전체 수정(PUT) 또는 일부 수정(PATCH) |
| 삭제 (Delete) | DELETE | /users/1 | 1번 사용자를 삭제함 |
---
...
| 4 | 일관된 인터페이스 (Uniform Interface) | 필수 |
| 5 | 계층 구조 (Layered System) | 필수 |
...
1. 자원의 식별 (URI 기반): URI는 동사가 아닌 명사 기반이어야 함
2. 표준 메서드 사용: GET, POST, PUT, DELETE 등 HTTP 표준 메서드로 동작 표현
3. 표준 문서 형태로 데이터 공유: 서버는 자원을 JSON, XML, YAML 등 표준 형태로 전달
4. 자기서술적 메시지(Self-descriptive message): 요청/응답 메시지는 스스로 해석 가능해야 함
```plain text
# URI 올바른 예 (명사형)
GET /users/10
...
```
자기서술적 메시지 예: 요청에 `Content-Type: application/json`을 포함하고, 응답에 `HTTP/1.1 201 Created`, `Location: /users/10` 등을 담아 메시지에 모든 의미가 담겨 문서 없이도 해석 가능해야 함.
---
...
| 규칙 | 설명 및 예시 |
| --- | --- |
| 자원(Resource) 명사 사용 | URI는 동작이 아닌 자원(명사)로 작성. 예) `/users`, `/products/123/comments` |
| 동사 대신 HTTP 메서드 사용 | URI에 동사를 쓰지 않고 GET/POST/PUT/DELETE로 동작 표현. 예) `GET /users` |
| 계층적 구조 | 자원 간 포함 관계는 URI로 계층 구조 표현. 예) `/users/123/orders/5` |
| 단수/복수 일관성 | 전체 집합은 복수형으로 표현 (단수형 가능하나 일관성 유지). 예) `/users`, `/products` |
| 쿼리 파라미터 | 필터, 정렬, 검색 등은 쿼리 파라미터 사용. 예) `/users?age=20&sort=name` |
| 상태코드 활용 | 처리 결과에 따라 적절한 HTTP 상태코드 반환. 예) `200`(성공), `201`(생성됨), `404`(없음), `400`(잘못된 요청) |
| 응답 포맷 | 보통 JSON 형식 사용. 예) `{ "id": 1, "name": "Alice" }` |
| Stateless | 각 요청은 독립적으로 처리, 서버는 클라이언트 상태 저장하지 않음 |
---
### REST API 설계 예시
...
| 단일 조회 | GET | /users/3 | id=3인 사용자 조회 |
| 전체 수정 | PUT | /users/3 | id=3인 사용자 정보 전체 수정 |
| 일부 수정 | PATCH | /users/3 | id=3인 사용자 정보 일부 수정 |
| 삭제 | DELETE | /users/3 | id=3인 사용자 삭제 |
게시판 관리
| 기능 | HTTP 메서드 | URI | 설명 |
| --- | --- | --- | --- |
| 게시글 목록 | GET | /posts | 게시글 전체 목록 |
| 게시글 생성 | POST | /posts | 게시글 등록 |
| 게시글 조회 | GET | /posts/10 | id=10 게시글 조회 |
| 댓글 목록 | GET | /posts/10/comments | 게시글 10번의 댓글들 |
...
| 특징 | 설명 |
| --- | --- |
| 경량 프레임워크 | 필요 모듈만 사용, 메모리 부담 적음 |
| 제어의 역전(IoC) | 객체 생성/관리를 프레임워크가 담당 |
| 의존성 주입(DI) | 객체 간 결합도 최소화, 유지보수성 향상 |
| AOP | 로깅·보안·트랜잭션 등 공통 기능 분리 |
| 모듈화 | Web, Data, Security 등 다양한 기능 모듈화 |
| 테스트 용이성 | DI를 통한 Mock 객체 활용, 유닛 테스트 용이 |
---
...
| --- | --- | --- |
| 등장 | 2003년 | 2014년 |
| 목적 | 유연하고 강력한 엔터프라이즈 개발 | 빠르고 쉽게 Spring 개발, 설정 최소화 |
| 주요 특징 | IoC/DI, AOP, 다양한 모듈 | 자동설정, 스타터, 내장 서버, 쉬운 실행 |
...
| 프로젝트 | 역할 |
| --- | --- |
| Spring Boot | 복잡한 환경 설정 자동화 |
| Spring Data JPA | DB 연동 표준화, 생산성 향상 |
| Spring Security | 인증/인가 보안 기능 |
| Spring Batch | 대용량 데이터 일괄 처리 |
| Spring Cloud | 마이크로서비스·클라우드 개발 |
| Spring WebFlux | 비동기/논블로킹 웹 개발 |
---
### \[참고\] 전통적인 Spring vs Spring Boot
| 비교 항목 | 전통적 Spring | Spring Boot |
| --- | --- | --- |
| 서버 동작 | 외부 WAS(WebSphere 등)에 WAR 배포 | 내장 서버(Tomcat 등) 포함 JAR 실행 |
| 설정 방식 | XML 또는 Java Config, 수동 설정 많음 | 자동 설정, Starter 의존성 |
| 패키징 | WAR | JAR |
| 배포 | WAS에 수동 배포 | `java -jar`로 바로 실행 |
| 시작 속도 | 느림 | 빠름 |
| 아키텍처 | MVC 위주 | REST API 위주 |
---
### 컴포넌트 스캔 (Component Scan)
스프링이 지정된 패키지 이하를 탐색하여 특정 어노테이션이 붙은 클래스를 찾아 스프링 빈(Bean)으로 자동 등록하는 기능임. 기본 스캔 범위는 `@SpringBootApplication`이 선언된 클래스의 하위 패키지 전체임.
`@SpringBootApplication`은 아래 3가지 어노테이션의 조합임.
| 구분 | 역할 |
| --- | --- |
| `@Configuration` | 자바 코드로 스프링 Bean 생성·조립 설정 |
| `@EnableAutoConfiguration` | 외부 패키지에서 필요한 클래스를 찾아 Bean 등록 |
| `@ComponentScan` | 해당 패키지 및 하위 패키지의 `@Component`, `@Service`, `@Repository` 등 자동 탐색 |
...
| 어노테이션 | 설명 | 용도 |
| --- | --- | --- |
| `@Component` | 범용 컴포넌트, 스프링 빈으로 자동 등록 | 공통 모듈, 유틸리티 클래스 |
| `@Controller` | MVC 패턴 컨트롤러, 웹 요청 처리 | REST API, 웹 페이지 처리용 컨트롤러 |
| `@RestController` | `@Controller` + `@ResponseBody`, REST API 특화 | RESTful 웹 서비스 컨트롤러 |
| `@Service` | 비즈니스 로직 담당 클래스 | 서비스 계층 |
| `@Repository` | 데이터 액세스 계층(DAO), 예외 변환 기능 | JPA, MyBatis 등 DB 연동 클래스 |
| `@Configuration` | 자바 기반 설정 클래스, `@Bean` 메서드 사용 가능 | 스프링 설정 및 빈 등록 |
| `@ConfigurationProperties` | 외부 설정값(프로퍼티 등)을 객체에 바인딩 | YAML, properties → 자바 객체 매핑 |
| `@EnableAutoConfiguration` | 스프링 부트 자동 설정 활성화 | `@SpringBootApplication`에 포함 |
---
...
```
`SpringApplication.run()`에 전달하는 `MyappApplication.class`가 스캔 시작점이 됨. 이 클래스가 위치한 패키지(`com.sk.skala.myapp`) 및 그 하위 패키지 전체가 자동 스캔 범위가 됨.
---
### Spring 개요
Java 기반 오픈소스 애플리케이션 프레임워크로, 엔터프라이즈급 애플리케이션 개발의 표준임. 복잡한 Java 개발을 단순화하는 컨테이너 역할을 함.
| 특징 | 설명 |
...
1. 클라이언트 HTTP Request 수신 → Tomcat이 `HttpServletRequest`/`HttpServletResponse` 생성
2. DispatcherServlet이 요청을 받음 (Front Controller 역할)
3. HandlerMapping에 URL 매핑 정보 조회 → 처리할 Controller 메서드 검색
4. HandlerAdapter가 해당 Controller 메서드 호출
5. Controller → Service → Repository 순서로 비즈니스 로직 실행
6. 결과(Model)를 ViewResolver에 전달 → View 렌더링 (MVC 방식) 또는 HTTPMessageConverter로 JSON 변환 (REST 방식)
7. 응답을 `HttpServletResponse`로 작성, 톰캣이 클라이언트로 전송
---
### REST API 기반 Spring Boot MVC — MVC 방식과의 차이
| 구분 | 스프링 MVC (View 반환) | REST API (JSON 반환) |
| --- | --- | --- |
| 어노테이션 | `@Controller` | `@RestController` (`@Controller` + `@ResponseBody`) |
| 반환 처리 | ViewResolver → Thymeleaf, JSP 등 HTML 렌더링 | HTTPMessageConverter → JSON/XML 변환 |
...
| 단계 | 클래스 | 설명 |
| --- | --- | --- |
| Request 수신 | `DispatcherServlet` | 모든 HTTP 요청을 처리하는 프론트 컨트롤러 |
| Handler Mapping | `RequestMappingHandlerMapping` | `@RequestMapping`, `@GetMapping` 등과 매핑되는 Controller 메서드 탐색 |
| Handler Adapter | `RequestMappingHandlerAdapter`, `HandlerMethodArgumentResolver` | Controller 메서드 실행, 파라미터 자동 주입 |
| Controller Bean | `@RestController` | 컨트롤러 역할의 Spring Bean |
| Reflection 호출 | `java.lang.reflect.Method#invoke` | 실제 Controller 메서드를 호출하는 Java Reflection API |
| Message Converter | `MappingJackson2HttpMessageConverter` | Java 객체를 JSON으로 변환 |
| Servlet Response | `HttpServletResponse` | 변환된 응답을 클라이언트에 전달 |
---
### \[참고\] Servlet 동작 구조
...
| View 구현체 | 설명 | 결과물 |
| --- | --- | --- |
| ThymeleafView | Thymeleaf 템플릿 엔진 | HTML |
| FreeMarkerView | FreeMarker 템플릿 엔진 | HTML |
| JstlView (JSP) | JSP + JSTL | HTML |
---
### Controller의 역할
...
컨트롤러를 지정하는 어노테이션은 `@Controller`와 `@RestController` 두 가지이며, 이 둘의 핵심 차이는 HTTP Response Body가 생성되는 방식임.
---
### @Controller vs @RestController
| 구분 | @Controller | @RestController |
| --- | --- | --- |
| 용도 | View(HTML) 반환 | JSON/XML 데이터 반환 |
| 동작 | ViewResolver → 템플릿 렌더링 | `@Controller` + `@ResponseBody` 조합과 동일 |
| JSON 반환 시 | 메서드에 `@ResponseBody` 추가 필요 | 클래스 전체에 자동 적용 |
...
| 모든 메서드 | `@RequestMapping` | method 속성으로 지정 | `@RequestMapping(value="/users", method=RequestMethod.GET)` |
---
### \[참고\] @RequestMapping 활용
`@RequestMapping`은 Spring MVC에서 HTTP 요청 URL과 메서드/클래스를 매핑하는 가장 기본적인 어노테이션임. `@GetMapping`, `@PostMapping` 등은 `@RequestMapping`의 method 속성을 축약한 버전임.
```plain text
@RequestMapping(value="/test", method=RequestMethod.GET)  =  @GetMapping("/test")
```
클래스 레벨에 `@RequestMapping`을 선언하면 해당 컨트롤러의 모든 메서드 URL 앞에 공통 prefix로 적용됨.
```java
@RestController
...
| 어노테이션 | 역할/용도 |
| --- | --- |
| `@PathVariable` | URL 경로의 값을 메서드 파라미터로 바인딩 |
| `@RequestParam` | 쿼리스트링, 폼 파라미터를 메서드 파라미터로 바인딩 |
| `@RequestBody` | HTTP 요청 Body(JSON, XML 등)를 자바 객체로 변환 |
| `@ModelAttribute` | 폼 데이터/쿼리 파라미터를 객체에 바인딩 |
| `@RequestHeader` | HTTP 요청 헤더 값을 파라미터로 바인딩 |
| `@CookieValue` | 쿠키 값을 파라미터로 바인딩 |
---
### @PathVariable
...
#### Repository 구현
`JpaRepository`를 상속받은 인터페이스를 선언하면 자동 구현체가 생성됨. Spring Data JPA의 경우 메서드 네이밍만으로 SQL을 자동 생성함.
```java
@Repository
