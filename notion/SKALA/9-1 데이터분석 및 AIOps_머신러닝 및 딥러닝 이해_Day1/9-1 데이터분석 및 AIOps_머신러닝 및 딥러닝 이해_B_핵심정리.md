---
title: "[9/1] 데이터분석 및 AIOps_머신러닝 및 딥러닝 이해_B_핵심정리"
notion_page_id: "3ce1d84b-f68e-8005-ae14-d7deaf58c5c0"
source_url: "https://app.notion.com/p/3ce1d84bf68e8005ae14d7deaf58c5c0"
synced_at: "2026-09-01T15:10:20+00:00"
content_sha256: "15d0b3b9a2f06ca1ad1da49c8e78886a3b85b4effec83dc1d8391e5b3ba91dd7"
---

# [9/1] 데이터분석 및 AIOps_머신러닝 및 딥러닝 이해_B_핵심정리

[[notion/SKALA/index|SKALA 학습 노트]]
> 원문: [Notion 페이지](https://app.notion.com/p/3ce1d84bf68e8005ae14d7deaf58c5c0)
>
...
- 선형회귀 + 활성함수: 위 결과값에 활성함수(Activation Function)를 한 번 더 적용하여 출력 `ŷ`를 만듦.
...
- 공식에서 `Wt`는 현재 위치(가중치), `η`는 한 걸음에 내려갈 거리(학습률), `∂E/∂Wt`는 경사면의 기울기, 새 `Wt`는 경사를 보고 한 걸음 내려간 다음 위치(새 가중치)를 의미함.
...
6. 수식 정리: `(X1*W2 + X2*W4)`를 `H2`로 치환하면 `∂E/∂W6 = Δ × H2`. 여기서 Δ는 오차(delta), H2는 은닉층의 출력값을 의미하며, 이 결과로 가중치 W6가 어떻게 조정되는지 확인할 수 있음.
...
#### 2-12. \[참고\] 가중치 업데이트 정리
공식 `*Wt = Wt - η × ∂E/∂Wt`에서 `Wt`는 현재 가중치, `η`는 학습률(한 걸음의 거리), `∂E/∂Wt`는 기울기(경사면의 기울기), 새 `Wt`는 업데이트된 가중치를 의미함.
...
```
- Epoch: 전체 데이터셋을 한 번 학습하는(파라미터를 한 차례씩 업데이트하는) 단위. 위 예시에서 `num_epochs=3`이면 20개 데이터 전체를 3번 반복 학습함.
- Batch: 학습 시 한 번에 처리하는 데이터 묶음. 만약 `batch_size=20`이라면 전체 데이터를 한 번에 처리하므로 1 epoch당 배치는 1개뿐임.
- Mini-Batch: Batch 중에서도 batch_size가 전체 데이터보다 작은 경우를 가리킴. 위 예시처럼 총 20개 데이터에 `batch_size=5`이면 1 epoch당 4개의 mini-batch로 나뉘어 학습되며, 이를 통해 메모리 절약, 학습 속도 향상, gradient 안정성 확보 효과를 얻을 수 있음.
