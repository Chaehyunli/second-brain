...
![1. 복잡한 함수 종속성 다듬기: 정규형 커버 (Canonical Cover)](assets/데이터베이스-설계--함수-종속성-이론과-스키마-분해/01-1.-복잡한-함수-종속성-다듬기-정규형-커버-Canonical-Cover.webp)
...
- 즉, F로부터 유추할 수 있는 모든 종속 관계를 Fc​로부터도 똑같이 유추 (F+=Fc+​).
...
결합 (Union Rule): - FD들의 결정자(LHS)가 같다면, 종속자(RHS)들을 하나로 합침 - 예를 들어, ${A \rightarrow B, A \rightarrow C}$는 A→BC로 합칠 수 있음

결정자(LHS)의 불필요한 속성 확인: - AB→C에서 A가 불필요한지 확인하려면, A를 제외한 B만으로 C를 결정할 수 있는지(B→C)를 기존 FD 집합으로 증명

종속자(RHS)의 불필요한 속성 확인: - A→BC에서 C가 불필요한지 확인하려면, 해당 FD를 A→B로 바꾼 상태에서 여전히 A→C가 전체 FD 집합으로부터 유추되는지 확인

불필요한 속성 제거: - 결정자 또는 종속자에 불필요한(extraneous) 속성이 있는지 확인하고 제거

불필요한 FD 제거: - 특정 FD 전체가 다른 FD들에 의해 유추될 수 있다면 해당 FD를 제거
...
테스트 방법: - 어떤 릴레이션 스키마 R이 BCNF를 만족하는지 확인하려면, 해당 스키마에 적용되는 모든 함수 종속성 α→β에 대해 결정자 α가 R의 슈퍼키(Superkey)인지 검사

주의할 점: - 최초의 스키마를 검증할 때는 주어진 FD 집합 F만 확인해도 충분하지만, 스키마가 분해된 후에는 F의 전체 폐포(F+)에 속하는 모든 FD를 고려 - 예를 들어 $F = {A \rightarrow B, B \rightarrow C}$가 주어졌을 때, $R(A, C, D)$라는 스키마는 F에 속한 FD만으로는 BCNF 위반이 없어 보이지만, $F^+$에 포함된 A→C에 의해 BCNF를 위반하게 됨

분해 알고리즘: - BCNF가 아닌 릴레이션을 발견하면, BCNF를 위반하는 FD(α→β)를 기준으로 $(\alpha \cup \beta)$와 (R−β) 두 개의 릴레이션으로 분해하는 과정을 반복

3NF (Third Normal Form)

특징: - BCNF보다 조금 완화된 정규형으로, BCNF 분해 시 발생할 수 있는 '종속성 보존 실패' 문제를 해결하기 위해 사용 - 항상 무손실 조인과 종속성 보존을 만족하는 분해가 가능

분해 알고리즘: 3NF 분해는 다음 단계를 따름.

주어진 FD 집합의 정규형 커버(Fc​)를 계산

Fc​에 있는 각 FD α→β에 대해, 하나의 릴레이션 스키마(α∪β)를 생성
