...
- **1. input()의 본질** — 성질: 무조건 **문자열(String)**로 읽음
- **2. split()의 역할** — 기능: 공백(스페이스, 탭, 엔터)을 기준으로 문자열을 자룸.
- **3. map(int, ...)** — 기능: 여러 개의 데이터를 한꺼번에 정수(int)로 변환.
- **4. map을 쓰는 이유 (장점)** — 만약 map을 쓰지 않는다면 아래와 같이 복잡하게 짜야 함.
