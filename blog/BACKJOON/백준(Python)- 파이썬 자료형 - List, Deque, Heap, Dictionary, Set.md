...
- **1. 리스트 (List) - 가장 기본** — 가장 많이 쓰이며, 스택(Stack) 기능을 포함.
- **2. 덱 (Deque) - BFS 필수템** — 큐(Queue)를 구현할 때 리스트(pop(0))를 쓰면 O(N)이라 느려짐
- **3. 힙 (Heap) - 우선순위 큐** — 다익스트라(최단거리) 알고리즘이나 상위 K번째 요소를 찾을 때 필수
- **4. 딕셔너리 (Dictionary) - 해시 맵** — 특정 데이터를 키(Key)값으로 빠르게 조회해야 할 때 씁니다.
