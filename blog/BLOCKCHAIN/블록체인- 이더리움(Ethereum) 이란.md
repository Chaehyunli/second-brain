---
title: "[블록체인] 이더리움(Ethereum) 이란?"
created: 2026-07-13
updated: 2026-07-13
type: blog-post
tags: ["blog", "technical-writing", "blockchain", "CS"]
category: "BLOCKCHAIN"
published: 2025-03-25
source_url: https://ch010104.tistory.com/26
---

# [블록체인] 이더리움(Ethereum) 이란?

## 원문

https://ch010104.tistory.com/26

## 핵심 요약

- **1. 이더리움이란?** — 창시자: 비탈릭 부테린 (Vitalik Buterin), 2013년 백서 발표 → 2015년 정식 런칭
- **1) 이더리움 블록체인** — 스마트 컨트랙트의 개발 및 실행이 가능한 분산형 오픈소스 플랫폼
- **2) Ether (ETH)** — 가치 교환 수단이자, **스마트 계약 실행 수수료(GAS)**로 사용
- **3) 스마트 컨트랙트 (Smart Contract)** — EVM 상에서 실행되며, 계약 실행 시 GAS 소비

## 관련 글

- [[blog/BLOCKCHAIN/index|BLOCKCHAIN]]
- [[blog/BLOCKCHAIN/블록체인- 비트코인(Bitcoin) 이란|[블록체인] 비트코인(Bitcoin) 이란?]]
- [[blog/BLOCKCHAIN/블록체인- 암호화 기술과 스마트 계약(Smart Contact)이란|[블록체인] 암호화 기술과 스마트 계약(Smart Contact)이란?]]
- [[blog/BLOCKCHAIN/블록체인- 블록체인 기술 설계 및 고려사항|[블록체인] 블록체인 기술 설계 및 고려사항]]
