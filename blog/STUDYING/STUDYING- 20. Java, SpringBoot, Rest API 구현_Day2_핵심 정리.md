---
title: "[STUDYING] 20. Java, SpringBoot, Rest API 구현_Day2_핵심 정리"
created: 2026-08-12
updated: 2026-08-12
type: blog-post
...
source_url: https://ch010104.tistory.com/337
---
# [STUDYING] 20. Java, SpringBoot, Rest API 구현_Day2_핵심 정리

## 원문

https://ch010104.tistory.com/337
...
일반 클래스에서도 return this를 활용하면 메서드 체이닝을 직접 구현할 수 있음.
...
Spring의 @Autowired로 의존성 주입, @RequestMapping으로 URL-메서드 매핑이 대표적인 예임
...
@Retention(RUNTIME)으로 설정해야 런타임에 isAnnotationPresent()로 어노테이션을 읽을 수 있음. SOURCE나 CLASS로 설정하면 런타임에는 어노테이션 정보가 사라짐.
...
@interface로 Annotation 정의

클래스/메서드/필드 등에 Annotation 적용

적용 범위에 따라 해석 주체가 달라짐
...
속성이 하나이고 이름이 value이면 @Hint("강한 힌트")처럼 이름 생략이 가능함.
...
RetentionPolicy.SOURCE를 사용하려면 AbstractProcessor를 상속 후 process 메서드로 구현함
