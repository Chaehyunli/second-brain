...
- **📌 ML(Machine Learning) 란?** — **머신러닝(ML)**은 컴퓨터가 명시적으로 프로그래밍되지 않고도 데이터를 기반으로 학습하고 스스로 패턴을 찾는 인공지능(AI) 기술!
- **1. ML을 왜 사용할까? ( 전통적인 학습 알고리즘 VS ML 개념)** — 사람이 직접 정의한 규칙과 조건문(If-Else)으로 설계됨
- **2. ML vs 전통적인 알고리즘 비교** — 사람이 직접 규칙을 설계하기 어려운 경우(예: 자연어 처리, 음성 인식)
- **3. 다양한 ML 적용 사례** — 제품 이미지 분석: CNNs, Transformers 활용
