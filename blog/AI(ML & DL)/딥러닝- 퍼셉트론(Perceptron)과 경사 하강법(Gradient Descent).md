...
![1. 목표: 점들을 가르는 최적의 직선 찾기](assets/딥러닝--퍼셉트론-Perceptron-과-경사-하강법-Gradient-Descent/01-1.-목표-점들을-가르는-최적의-직선-찾기.webp)
...
![2. 퍼셉트론 알고리즘의 원리](assets/딥러닝--퍼셉트론-Perceptron-과-경사-하강법-Gradient-Descent/02-2.-퍼셉트론-알고리즘의-원리.webp)
...
w_1,w_2 : 각 입력(x_1,x_2)에 대한 가중치(weight) - 이 가중치들이 직선의 기울기를 결정

b : **편향(bias)** - 직선의 위치(y절편)를 조절

sign() : 괄호 안의 계산 결과가 양수이면 +1, 음수이면 -1을 반환하는 함수

학습 규칙 (가중치 업데이트)

핵심은 '잘못 분류했을 때' 가중치(w)와 편향(b)을 어떻게 업데이트 하느냐에 있음

빨간 점(-1)을 파란 영역(+1)으로 잘못 예측한 경우

직선을 빨간 점 쪽으로 더 가깝게 이동시켜야 함

이를 위해 w_1x_1+w_2x_2+b의 값을 더 작게 만들어야 함

따라서, 가중치와 편향에서 일정 값을 빼줌

w_new=w_old−α⋅p (p는 점의 좌표)

b_new=b_old−α

파란 점(+1)을 빨간 영역(-1)으로 잘못 예측한 경우

직선을 파란 점 쪽으로 더 가깝게 이동시켜야 함

w_1x_1+w_2x_2+b의 값을 더 크게 만들어야 함
...
α(알파)는 **학습률(learning rate)**이라고 하며, 직선을 한 번에 얼마나 많이 움직일지를 결정하는 값임
...
![3. 퍼셉트론의 한계와 경사 하강법](assets/딥러닝--퍼셉트론-Perceptron-과-경사-하강법-Gradient-Descent/03-3.-퍼셉트론의-한계와-경사-하강법.webp)
...
가중치 업데이트: 비용 함수를 가중치(W)로 미분하여 기울기를 구하고, 이 기울기의 반대 방향으로 학습률(α)만큼 가중치를 이동시김
