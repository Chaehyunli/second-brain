...
- **📦 NumPy란?** — NumPy는 Python에서 과학/수치 계산을 위한 가장 기본적인 라이브러리.
- **배열 생성 방법** — np.zeros(): 0으로 채워진 배열 생성
- **np.arange() vs np.linspace()** — np.arrange(start, end) 형식 - 범위의 시작 start와 end를 이용하여 1씩 증가.
- **np.fromfunction(): 함수 기반 배열 생성** — 배열 생성후에 초기값을 채우는 것을 함수를 이용하여 정함
