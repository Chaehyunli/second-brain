...
updated: 2026-07-13
type: blog-post
tags: ["blog", "technical-writing", "java"]
category: "JAVA"
...
set 메서드 메서드 설명 예제 add(E e) 요소 추가 (중복된 값이면 false 반환) set.add("Apple"); remove(Object o) 특정 요소 삭제 set.remove("Apple"); contains(Object o) 특정 요소가 존재하는지 확인 set.contains("Apple"); size() 저장된 요소 개수 반환 set.size(); isEmpty() Set이 비어있는지 확인 set.isEmpty(); clear() 모든 요소 삭제 set.clear(); iterator() Iterator 반환하여 요소 순회 Iterator<String> it = set.iterator(); toArray() Set을 배열로 변환 String[] arr = set.toArray(new String[0]);
