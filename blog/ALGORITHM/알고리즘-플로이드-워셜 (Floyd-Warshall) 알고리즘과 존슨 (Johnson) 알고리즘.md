...
- **플로이드-워셜 (Floyd-Warshall) 알고리즘** — - 모든 쌍 최단 경로(APSP)를 동적 계획법(Dynamic Programming) 기반으로 해결하는 알고리즘
- **존슨 (Johnson) 알고리즘** — - 음수 가중치가 있지만 음수 사이클은 없는 유향 그래프에서 모든 쌍 최단 경로(APSP)를 효율적으로 해결하는 알고리즘
